...
###############################################################################
# build_oe4t_jetson_multi_platform_v44.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v44.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v44.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v44.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v44.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v44.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v44.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v44.sh
#
# Important:
...
#
# v44 notes:
#   - Keeps the validated v43/v42 and prior platform/build/bundle fixes intact.
#   - Replaces the invalid v43 gcc-runtime-native dependency with a recipe-local
#     libcxx-native libstdc++ staging shim, because this branch provides
#     gcc-runtime but not gcc-runtime-native.
#   - Adds an empty-package/RPROVIDES compatibility shim for compiler-rt,
#     compiler-rt-dev, and compiler-rt-staticdev package resolution on Thor.
#
# v43 notes:
#   - Keeps the validated v42 OpenSSL and prior platform/build/bundle fixes intact.
#   - Attempted to fix libcxx-native by depending on gcc-runtime-native. v44
#     supersedes this because gcc-runtime-native is not provided on Thor.
#
# v42 notes:
#   - Keeps the validated v41 platform/build/bundle fixes intact.
#   - Fixes OpenSSL 3.5.x target do_install failures seen on high-core x64
#     Ubuntu 22.04 hosts by serializing target OpenSSL compile/install make.
#     The failure signature is make -j<N> install_sw followed by missing
#     crypto/*/*.d.tmp dependency-file rename errors.
#
# v41 notes:
#   - Keeps the validated v40 platform/build fixes intact.
#   - Unifies successful build-completion output across all platforms.
#   - Adds optional --bundle packaging of the selected primary flashing image
#     into a platform-named .tar.gz archive in the invocation directory.
#
# v40 fixes:
#   - Carries forward the v35 Thor CUDA duplicate-header producer/manifest fix.
#   - Adds an OpenCV-specific Thor CUDA sysroot bbappend. OpenCV/CMake finds
#     native nvcc, then CMake's CUDA::cudart/CUDA::cuda_driver imported targets
#     validate this path:
#       recipe-sysroot-native/usr/local/cuda-13.0/include
#     On this branch that directory can be absent even though the target CUDA
#     headers exist. v40 creates a recipe-local native CUDA include symlink back
#     to the target recipe sysroot before OpenCV CMake generation validates the
#     imported targets.
#   - Bumps all Thor CUDA/OpenCV/FreeImage stamps and PR suffixes to v40 so the
#     repairs run even if v35 already stamped the tree.
###############################################################################
...
Examples:
  ...
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v44.sh
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v44.sh
  ./build_oe4t_jetson_multi_platform_v44.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v44.sh --bundle
...
append_once_block() {
    ...
    sed -i '/^# BEGIN generated by build_oe4t_jetson_multi_platform_v3[0-9]\.sh thor cuda overlap guard$/,/^# END generated by build_oe4t_jetson_multi_platform_v3[0-9]\.sh thor cuda overlap guard$/d' "$file"
    sed -i '/^# BEGIN generated by build_oe4t_jetson_multi_platform_v4[0-9]\.sh thor cuda overlap guard$/,/^# END generated by build_oe4t_jetson_multi_platform_v4[0-9]\.sh thor cuda overlap guard$/d' "$file"
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v44.sh thor cuda overlap guard

# Thor / CUDA 13 native sysroot collision guard.
# Current BitBake treats the old SSTATE_DUPWHITELIST variable as a fatal parse
# error, so keep only the renamed variable.
...
# END generated by build_oe4t_jetson_multi_platform_v44.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v44.sh.
# OpenCV is a target recipe, but CMake's FindCUDAToolkit can derive imported
# CUDA target include directories from native nvcc. On Thor CUDA 13 this can
# produce CUDA::cudart/CUDA::cuda_driver with:
#   ${RECIPE_SYSROOT_NATIVE}/usr/local/cuda-13.0/include
# even when the real headers are in ${RECIPE_SYSROOT}. CMake validates imported
# INTERFACE_INCLUDE_DIRECTORIES during generation, so the path must exist.
#
# v40 also sanitizes opencv4.pc before package QA. v37 fixed configure, but
# OpenCV 4.13.0 can still package TMPDIR/sysroot paths into opencv-dev via
# /usr/lib/pkgconfig/opencv4.pc, causing buildpaths QA to fail.
PR:append = ".jetsonbuilder40"
...
jetson_builder_fix_opencv_cuda_native_include() {
    ...
    echo "jetson-builder v40 OpenCV CUDA native root: ${native_root}"
    echo "jetson-builder v40 OpenCV CUDA target root: ${target_root}"
    echo "jetson-builder v40 OpenCV CUDA selected target include: ${target_include:-none}"
    ls -ld "${native_root}" || true
    ls -ld "${native_include}" || true

    if [ ! -d "${native_include}" ]; then
        bbfatal "jetson-builder v40: ${native_include} still does not exist; cannot satisfy CMake CUDA imported target validation"
    fi
}
...
jetson_builder_sanitize_opencv_pkgconfig() {
    ...
    if [ ! -f "${pc_file}" ]; then
        echo "jetson-builder v40 OpenCV pkg-config sanitize: ${pc_file} not found; skipping"
        return 0
    fi

    tmp_file="${pc_file}.jetsonbuilder40"
    ...
    awk \
        ...
        BEGIN {
            ...
            roots[3] = builddir
            roots[4] = sourcedir
            roots[5] = recipe_sysroot
            roots[6] = recipe_sysroot_native
            roots[7] = staging_target
            roots[8] = staging_native
        }
        function is_bad_token(token, i) {
            for (i = 1; i <= 8; i++) {
                if (roots[i] != "" && index(token, roots[i]) > 0) {
                    return 1
                }
            }
            return 0
        ...
        /^includedir=/ {
            print "includedir=${prefix}/include/opencv4"
            next
        }
    ...
    if cmp -s "${pc_file}" "${tmp_file}"; then
        rm -f "${tmp_file}"
        echo "jetson-builder v40 OpenCV pkg-config sanitize: no changes needed for ${pc_file}"
    else
        mv "${tmp_file}" "${pc_file}"
        echo "jetson-builder v40 OpenCV pkg-config sanitize: sanitized ${pc_file}"
    fi
}
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v44.sh.
BBPATH .= ":\${LAYERDIR}"
...
# OpenSSL 3.5.x can race during do_install on high-core hosts because
# install_sw may enter recursive build targets in parallel.  The observed
# signature is a missing crypto/*/*.d.tmp file during dependency rename.
# Keep this narrow to OpenSSL rather than reducing global build parallelism.
PR:append = ".jetsonbuilder42"
PARALLEL_MAKE = "-j1"
...
# Generated by build_oe4t_jetson_multi_platform_v44.sh.
#
# Fix libcxx-native configure failure:
#
#   /tmp/hosttools/ld: cannot find -lstdc++: No such file or directory
#
# v43 tried DEPENDS:append:class-native = " gcc-runtime-native", but this Thor
# branch has gcc-runtime only, not gcc-runtime-native.  Keep this local and
# surgical: before libcxx-native configure, expose the host/native GNU C++
# runtime inside this recipe's native sysroot so clang++ can pass CMake's
# compiler sanity link before libc++ itself has been built.
PR:append = ".jetsonbuilder44"
...
jetson_builder_stage_libstdcxx_for_libcxx_native() {
    ...
    if [ -z "${libstdcxx}" ] || [ ! -e "${libstdcxx}" ]; then
        bbfatal "jetson-builder v44: could not locate host/native libstdc++.so for libcxx-native CMake sanity link"
    fi
    ...
    bbnote "jetson-builder v44: staged libcxx-native libstdc++ link: ${native_libdir}/libstdc++.so -> ${libstdcxx}"
}
...
# Generated by build_oe4t_jetson_multi_platform_v44.sh.
#
# On the Thor branch, package resolution can ask for compiler-rt,
# compiler-rt-dev, and compiler-rt-staticdev even when the recipe's split
# packages are empty for the selected configuration.  Allow the package names
# to exist and explicitly RPROVIDE the canonical names so do_rootfs/dependency
# resolution does not remove the chain as unbuildable.
PR:append = ".jetsonbuilder44"
...
# v40 is intentionally surgical: remove only the known duplicate header from
# cuda-nvcc-native, and preserve the top-level CUDA include directory that
# CMake's CUDA::cudart/CUDA::cuda_driver imported targets reference.
PR:append = ".jetsonbuilder40"
...
# v40 removes only the exact duplicate fatbinary_section.h ownership entry from
# cuda-nvcc-native. Broader include-directory pruning breaks OpenCV because
# CMake's CUDA imported targets require /usr/local/cuda-13.0/include to exist.
PR:append = ".jetsonbuilder40"
...
python jetson_builder_prune_cuda_nvcc_native_component () {
    ...
    removed_files = []
    patterns = [
        os.path.join(tmpdir, 'sysroots-components', '*', 'cuda-nvcc-native', 'usr', 'local', 'cuda-13.0', 'targets', 'sbsa-linux', 'include', 'fatbinary_section.h'),
    ]
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            for line in lines:
                ...
                if normalized.endswith(duplicate_suffix):
                    changed = True
                    continue
                kept.append(line)

            if changed:
                tmp_manifest = manifest + '.jetsonbuilder40.tmp'
...
EOF_CUDA_COMPILER
    ...
    if [ "$platform" = "thor" ] && [ "$has_opencv_recipe" = "1" ]; then
        ...
        if [ "$platform" = "thor" ]; then
            warn "Skipping Thor OpenCV CUDA sysroot fix; no matching OpenCV recipe was found."
        fi
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v44.sh.
# Preserve FreeImage's original include/macro flags and append PIC through
# Makefile.gnu instead of replacing CFLAGS/CXXFLAGS on the make command line.
PR:append = ".jetsonbuilder40"
...
# jetson-builder v40 force PIC without replacing FreeImage include/macro flags
...
ensure_active_freeimage_pic_fix() {
    ...
    local env_file="/tmp/jetson-builder-freeimage-v40-env.txt"
    if ! bitbake -e freeimage > "$env_file" 2>/tmp/jetson-builder-freeimage-v40-env.err; then
    ...
    if [ -n "$recipe_file" ] && [ -f "$recipe_file" ]; then
        ...
        log "Installing FreeImage v40 PIC bbappend next to active recipe: $recipe_file"
...
if components.exists():
    for comp in components.glob('*/cuda-nvcc-native'):
        ...
        if dup.exists() or dup.is_symlink():
            try:
                dup.unlink()
                removed_files.append(str(dup))
            except OSError:
                pass
        # Do not remove cuda-13.0/include. OpenCV/CMake CUDA imported targets
        # validate that path in recipe-sysroot-native.
...
for manifest in manifest_candidates:
    ...
    for line in lines:
        ...
        kept.append(line)
    if changed:
        tmp_manifest = manifest.with_suffix(manifest.suffix + '.jetsonbuilder40.tmp')
...
if rewritten:
    ...
    for item in rewritten[:60]:
        print('  ' + item)
if not (removed_files or rewritten):
    print('No duplicate cuda-nvcc-native fatbinary_section.h entries found to prune yet.')
...
clean_libcxx_native_state_if_needed() {
    ...
    local build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    local libcxx_fix_bbappend="$fix_layer/recipes-devtools/clang/libcxx_%.bbappend"
    local stamp_dir="$build_abs_dir/conf/.jetson-builder-stamps"
    local stamp_file="$stamp_dir/libcxx-native-stdcxx-clean-v44.sha256"

    [ -f "$libcxx_fix_bbappend" ] || return 0
    mkdir -p "$stamp_dir"

    local current_hash
    current_hash="$(sha256sum "$libcxx_fix_bbappend" | awk '{print $1}')"

    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "libcxx-native stdc++ cleansstate guard already applied for the current v44 fix."
        return 0
    fi

    log "Cleaning stale libcxx-native state so the v44 stdc++ sysroot shim is used"
    bitbake -c cleansstate libcxx-native || true
    bitbake -c cleansstate compiler-rt || true
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_dir="$build_abs_dir/conf/.jetson-builder-stamps"
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v40.sha256"
    mkdir -p "$stamp_dir"

    local hash_inputs=()
    hash_inputs+=("$cuda_fix_bbappend")
    [ -f "$cuda_compiler_fix_bbappend" ] && hash_inputs+=("$cuda_compiler_fix_bbappend")
    [ -f "$opencv_fix_bbappend" ] && hash_inputs+=("$opencv_fix_bbappend")
    [ -n "$local_conf" ] && [ -f "$local_conf" ] && hash_inputs+=("$local_conf")

    local current_hash
    current_hash="$(cat "${hash_inputs[@]}" | sha256sum | awk '{print $1}')"
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v40 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA/OpenCV state so the v40 CUDA include repair is used"
...
clean_freeimage_state_if_needed() {
    ...
    if ! bitbake -e freeimage >/tmp/jetson-builder-freeimage-v40-clean-env.txt 2>/dev/null; then
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v40.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v40 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v40 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    ...
    log "Validating FreeImage v40 PIC fix only if freeimage is active"
    ...
    if ! grep -q 'jetson_builder_force_freeimage_pic' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage is active, but the v40 PIC bbappend hook is not visible in BitBake metadata."
    fi

    log "Forcing FreeImage compile validation with v40 PIC fix"
    bitbake -c cleansstate freeimage
...
validate_opencv_cuda_sysroot_fix() {
    ...
    log "Validating OpenCV v40 CUDA native include sysroot fix if opencv is active"
    ...
    if ! grep -q 'jetson_builder_fix_opencv_cuda_native_include' /tmp/jetson-builder-opencv-env.txt; then
        die "OpenCV is active, but the v40 CUDA sysroot bbappend hook is not visible in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_sanitize_opencv_pkgconfig' /tmp/jetson-builder-opencv-env.txt; then
        die "OpenCV is active, but the v40 OpenCV pkg-config sanitizer hook is not visible in BitBake metadata."
    fi
    bitbake -c clean opencv || true

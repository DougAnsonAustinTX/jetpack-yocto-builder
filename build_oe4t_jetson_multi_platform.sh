...
###############################################################################
# build_oe4t_jetson_multi_platform_v15.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v15.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v15.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v15.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v15.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v15.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v15.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v15.sh
#
# Important:
...
ensure_foreign_binary_support() {
    ...
    if ! command -v qemu-x86_64-static >/dev/null 2>&1 && ! command -v qemu-x86_64 >/dev/null 2>&1; then
        warn "qemu-x86_64 is not available. Install qemu-user-static/binfmt-support if mkbootimg still fails with Exec format error."
    fi
}
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v15.sh.

...
###############################################################################
# build_oe4t_jetson_multi_platform_v19.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v19.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v19.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v19.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v19.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v19.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v19.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v19.sh
#
# Important:
...
#
# v19 fixes:
#   - Fixes the tegra-flashtools bbappend parse error by appending shell to the
#     shell do_install task. V18 used a BitBake Python do_install append against
#     a shell task, so BitBake tried to parse Python as shell.
#   - Keeps the mkbootimg wrapper body inside a plain POSIX sh heredoc in the
#     generated bbappend.
#
# v18 fixes superseded:
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v19.sh.
...
#
# Important BitBake syntax detail:
# tegra-flashtools-native's upstream do_install is a shell task. Append shell
# here too. Do not use "python do_install:append()" for this recipe; newer
# BitBake will try to assemble/parse the final do_install body as shell and fail
# on Python syntax during parsing.
...
do_install:append() {
    ...
    cat > "$mkbootimg_path" <<'JETSONBUILDER_MKBOOTIMG_WRAPPER'
#!/bin/sh
set -u
...
exit 126
JETSONBUILDER_MKBOOTIMG_WRAPPER

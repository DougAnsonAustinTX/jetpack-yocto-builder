...
###############################################################################
# build_oe4t_jetson_multi_platform_v18.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v18.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v18.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v18.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v18.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v18.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v18.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v18.sh
#
# Important:
...
#
# v18 fixes:
#   - Rewrites the tegra-flashtools mkbootimg wrapper bbappend generation to use
#     a BitBake Python do_install append instead of a shell heredoc. BitBake was
#     parsing shell lines from the embedded wrapper body as recipe syntax.
#
# v16 fixes:
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v18.sh.
...
# On ARM64 build hosts, NVIDIA's tegra-flash mkbootimg helper may be staged in
# a form that the host cannot execute directly. Keep the workaround scoped to
# tegra-flashtools-native and install a small launcher that detects the staged
# payload before choosing direct execution or a qemu user-mode fallback.
#
# Do not generate this wrapper with a shell heredoc inside do_install. BitBake's
# parser can mistake embedded shell-function braces/case blocks for recipe
# syntax. Generate the wrapper from a Python list of quoted lines instead.
...
python do_install:append() {
    ...
    mkbootimg_path = os.path.join(dvar, bindir.lstrip('/'), 'tegra-flash', 'mkbootimg')
    real_path = mkbootimg_path + '.real'

    if not os.path.exists(mkbootimg_path):
        return

    # Idempotent for rebuilds after a partially completed workdir.
    if os.path.exists(real_path):
        try:
            os.remove(mkbootimg_path)
        except FileNotFoundError:
            pass
    else:
        os.rename(mkbootimg_path, real_path)
    ...
    wrapper_lines = [
        ...
        '        ;;',
        '    *"ELF"*"ARM"*)',
        '        case "$host_arch" in arm*|armhf) exec "$real" "$@" ;; esac',
    ...
    with open(mkbootimg_path, 'w') as f:
        f.write('\n'.join(wrapper_lines))
        f.write('\n')

    os.chmod(mkbootimg_path, 0o755)
    current_mode = os.stat(real_path).st_mode
    os.chmod(real_path, current_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
}

...
###############################################################################
# build_oe4t_jetson_multi_platform_v43.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v43.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v43.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v43.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v43.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v43.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v43.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v43.sh
#
# Important:
...
#   - Fixes libcxx-native do_configure failures where clang++ falls back to
#     -lstdc++ during CMake compiler sanity checks, but libstdc++ is missing
#     from the native recipe sysroot.
#
# v42 notes:
...
Examples:
  ...
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v43.sh
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v43.sh
  ./build_oe4t_jetson_multi_platform_v43.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v43.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v43.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v43.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v43.sh.
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v43.sh.
BBPATH .= ":\${LAYERDIR}"
...
#
# libcxx-native configures with clang++, and CMake's compiler sanity check links
# a trivial C++ program before libc++ itself is available. clang++ therefore
# falls back to the GNU C++ runtime, so make the native GCC runtime available in
# this recipe sysroot.
PR:append = ".jetsonbuilder43"

DEPENDS:append:class-native = " gcc-runtime-native"

jetson_builder_validate_libcxx_native_stdlib() {
    if [ "${PN}" != "libcxx-native" ]; then
        return 0
    fi

    found_libstdcxx="$(find "${RECIPE_SYSROOT_NATIVE}" \
        \( -name 'libstdc++.so' -o -name 'libstdc++.so.*' -o -name 'libstdc++.a' \) \
        -print -quit 2>/dev/null || true)"

    if [ -z "${found_libstdcxx}" ]; then
        bbfatal "jetson-builder v43: libcxx-native still has no libstdc++ in RECIPE_SYSROOT_NATIVE after adding gcc-runtime-native"
    fi

    bbnote "jetson-builder v43: libcxx-native found native GNU C++ runtime: ${found_libstdcxx}"
}

do_configure[prefuncs] += "jetson_builder_validate_libcxx_native_stdlib "
...
clean_libcxx_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/libcxx-native-stdcxx-clean-v43.sha256"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "libcxx-native stdc++ cleansstate guard already applied for the current v43 fix."
        return 0
    fi

    log "Cleaning stale libcxx-native state so the v43 stdc++ dependency fix is used"
    bitbake -c cleansstate libcxx-native || true
    bitbake -c cleansstate gcc-runtime-native || true

...
###############################################################################
# build_oe4t_jetson_multi_platform_v45.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v45.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v45.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v45.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v45.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v45.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v45.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v45.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v45.sh --bundle
#
# Important:
...
#
# v45 notes:
#   - Keeps the v44 platform/build/bundle fixes intact.
#   - Adds an e2fsprogs 1.47.x shared-library relink guard for the Thor branch.
#   - Serializes e2fsprogs compile/install narrowly to avoid incomplete
#     libext2fs.so creation under high parallelism.
#   - Removes stale e2fsprogs libext2fs shared artifacts before configure and
#     compile, then validates that libext2fs.so exports the internal symbols
#     needed by e2fsck before do_install can fail late.
#   - Cleans stale e2fsprogs state once when the v45 fix content changes.
#
# v44 notes:
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v45.sh
  ./build_oe4t_jetson_multi_platform_v45.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v45.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v45.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v45.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v45.sh.
PR:append = ".jetsonbuilder45"
...
jetson_builder_fix_opencv_cuda_native_include() {
    ...
    echo "jetson-builder v45 OpenCV CUDA native root: ${native_root}"
    echo "jetson-builder v45 OpenCV CUDA target root: ${target_root}"
    echo "jetson-builder v45 OpenCV CUDA selected target include: ${target_include:-none}"
    ls -ld "${native_root}" || true
    ls -ld "${native_include}" || true

    if [ ! -d "${native_include}" ]; then
        bbfatal "jetson-builder v45: ${native_include} still does not exist; cannot satisfy CMake CUDA imported target validation"
    fi
}

jetson_builder_sanitize_opencv_pkgconfig() {
    pc_file="${D}${libdir}/pkgconfig/opencv4.pc"
    [ -f "${pc_file}" ] || { echo "jetson-builder v45 OpenCV pkg-config sanitize: ${pc_file} not found; skipping"; return 0; }
    tmp_file="${pc_file}.jetsonbuilder45"
    ...
    if cmp -s "${pc_file}" "${tmp_file}"; then
        rm -f "${tmp_file}"
        echo "jetson-builder v45 OpenCV pkg-config sanitize: no changes needed for ${pc_file}"
    else
        mv "${tmp_file}" "${pc_file}"
        echo "jetson-builder v45 OpenCV pkg-config sanitize: sanitized ${pc_file}"
    fi
}
...
jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        sed -i \
            ...
            -e '/jetson-builder v45 force PIC/,+4d' \
            "${S}/Makefile.gnu"
        cat >> "${S}/Makefile.gnu" <<'EOF_MAKEFILE_PIC'

# jetson-builder v45 force PIC without replacing FreeImage include/macro flags
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v45.sh.
BBPATH .= ":\${LAYERDIR}"
...
jetson_builder_e2fsprogs_remove_stale_libext2fs() {
...
jetson_builder_e2fsprogs_validate_libext2fs() {
    for sym in \
        ...
        if ! jetson_builder_e2fsprogs_symbol_defined "${sym}"; then
            echo "jetson-builder v45 e2fsprogs: libext2fs candidates:" >&2
            ls -l "${B}/lib/ext2fs/libext2fs.so"* "${B}/lib/libext2fs.so"* 2>/dev/null || true
            bbfatal "jetson-builder v45: libext2fs.so is missing required symbol ${sym}; refusing to continue to e2fsck/do_install link"
        fi
    done
    bbnote "jetson-builder v45: libext2fs.so exports required e2fsck symbols"
}
...
jetson_builder_stage_libstdcxx_for_libcxx_native() {
    ...
    [ -n "${libstdcxx}" ] && [ -e "${libstdcxx}" ] || bbfatal "jetson-builder v45: could not locate host/native libstdc++.so for libcxx-native CMake sanity link"
    ...
    bbnote "jetson-builder v45: staged libcxx-native libstdc++ link: ${native_libdir}/libstdc++.so -> ${libstdcxx}"
}
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            for line in lines:
                ...
                kept.append(line)
            if changed:
                tmp_manifest = manifest + '.jetsonbuilder45.tmp'
...
ensure_active_freeimage_pic_fix() {
    ...
    local env_file="/tmp/jetson-builder-freeimage-v45-env.txt"
    if ! bitbake -e freeimage > "$env_file" 2>/tmp/jetson-builder-freeimage-v45-env.err; then
    ...
    if [ -n "$recipe_file" ] && [ -f "$recipe_file" ]; then
        ...
        log "Installing FreeImage v45 PIC bbappend next to active recipe: $recipe_file"
...
for manifest in manifest_candidates:
    ...
    for line in lines:
        ...
        kept.append(line)
    if changed:
        tmp_manifest = manifest.with_suffix(manifest.suffix + '.jetsonbuilder45.tmp')
...
clean_recipe_state_if_fix_changed() {
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "$stamp_name cleansstate guard already applied for the current v45 fix."
        return 0
    fi
...
clean_libcxx_native_state_if_needed() {
    ...
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "libcxx-native-stdcxx-clean-v45" "libcxx-native compiler-rt" \
...
clean_e2fsprogs_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "e2fsprogs-libext2fs-clean-v45" "e2fsprogs" \
...
clean_thor_cuda_native_state_if_needed() {
    ...
    clean_recipe_state_if_fix_changed "$build_abs_dir" "thor-cuda-native-clean-v45" "cuda-nvcc-native cuda-nvcc-headers-native cuda-compiler-native" \
...
clean_freeimage_state_if_needed() {
    local platform="$1" distro_dir="$2"
    [ "$platform" = "thor" ] || return 0
    if ! bitbake -e freeimage >/tmp/jetson-builder-freeimage-v45-clean-env.txt 2>/dev/null; then
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v45.sha256"
    mkdir -p "$stamp_dir"
    local current_hash
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v45 fix."
        return 0
    fi
    log "Cleaning stale FreeImage state so the v45 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    local platform="$1"
    [ "$platform" = "thor" ] || return 0
    log "Validating FreeImage v45 PIC fix only if freeimage is active"
    ...
    grep -q 'jetson_builder_force_freeimage_pic' /tmp/jetson-builder-freeimage-env.txt || die "FreeImage is active, but the v45 PIC bbappend hook is not visible in BitBake metadata."
    log "Forcing FreeImage compile validation with v45 PIC fix"
    bitbake -c cleansstate freeimage
...
validate_opencv_cuda_sysroot_fix() {
    local platform="$1"
    [ "$platform" = "thor" ] || return 0
    log "Validating OpenCV v45 CUDA native include sysroot fix if opencv is active"
    ...
    grep -q 'jetson_builder_fix_opencv_cuda_native_include' /tmp/jetson-builder-opencv-env.txt || die "OpenCV is active, but the v45 CUDA sysroot bbappend hook is not visible in BitBake metadata."
    grep -q 'jetson_builder_sanitize_opencv_pkgconfig' /tmp/jetson-builder-opencv-env.txt || die "OpenCV is active, but the v45 OpenCV pkg-config sanitizer hook is not visible in BitBake metadata."
...
validate_e2fsprogs_libext2fs_fix() {
    local platform="$1"
    [ "$platform" = "thor" ] || return 0
    log "Validating e2fsprogs v45 libext2fs guard is visible"
    ...
    grep -q 'jetson_builder_e2fsprogs_validate_libext2fs' /tmp/jetson-builder-e2fsprogs-env.txt || die "e2fsprogs is active, but the v45 libext2fs validation hook is not visible in BitBake metadata."
}

...
###############################################################################
# build_oe4t_jetson_multi_platform_v37.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v37.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v37.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v37.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v37.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v37.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v37.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v37.sh
#
# Important:
...
#     headers exist. v37 creates a recipe-local native CUDA include symlink back
#     to the target recipe sysroot before OpenCV CMake generation validates the
#     imported targets.
#   - Bumps all Thor CUDA/OpenCV/FreeImage stamps and PR suffixes to v37 so the
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v37.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v37.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v37.sh.
...
jetson_builder_fix_opencv_cuda_native_include() {
    ...
    echo "jetson-builder v37 OpenCV CUDA native root: ${native_root}"
    echo "jetson-builder v37 OpenCV CUDA target root: ${target_root}"
    echo "jetson-builder v37 OpenCV CUDA selected target include: ${target_include:-none}"
    ls -ld "${native_root}" || true
    ls -ld "${native_include}" || true

    if [ ! -d "${native_include}" ]; then
        bbfatal "jetson-builder v37: ${native_include} still does not exist; cannot satisfy CMake CUDA imported target validation"
    fi
}
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v37.sh.
BBPATH .= ":\${LAYERDIR}"
...
# v37 is intentionally surgical: remove only the known duplicate header from
...
# v37 removes only the exact duplicate fatbinary_section.h ownership entry from
...
# jetson-builder v37 force PIC without replacing FreeImage include/macro flags
...
ensure_active_freeimage_pic_fix() {
    ...
    local env_file="/tmp/jetson-builder-freeimage-v37-env.txt"
    if ! bitbake -e freeimage > "$env_file" 2>/tmp/jetson-builder-freeimage-v37-env.err; then
    ...
    if [ -n "$recipe_file" ] && [ -f "$recipe_file" ]; then
        ...
        log "Installing FreeImage v37 PIC bbappend next to active recipe: $recipe_file"
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v37.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v37 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA/OpenCV state so the v37 CUDA include repair is used"
...
clean_freeimage_state_if_needed() {
    ...
    if ! bitbake -e freeimage >/tmp/jetson-builder-freeimage-v37-clean-env.txt 2>/dev/null; then
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v37.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v37 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v37 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    ...
    log "Validating FreeImage v37 PIC fix only if freeimage is active"
    ...
    if ! grep -q 'jetsonbuilder37' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage is active, but the v37 PIC bbappend is not active in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_force_freeimage_pic' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v37 PIC patch hook is not visible in BitBake metadata."
    fi

    log "Forcing FreeImage compile validation with v37 PIC fix"
    bitbake -c cleansstate freeimage
...
validate_opencv_cuda_sysroot_fix() {
    ...
    log "Validating OpenCV v37 CUDA native include sysroot fix if opencv is active"
    ...
    if ! grep -q 'jetsonbuilder37' /tmp/jetson-builder-opencv-env.txt; then
        die "OpenCV is active, but the v37 CUDA sysroot bbappend is not active in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_fix_opencv_cuda_native_include' /tmp/jetson-builder-opencv-env.txt; then
        die "OpenCV v37 CUDA native include hook is not visible in BitBake metadata."
    fi
    bitbake -c clean opencv || true

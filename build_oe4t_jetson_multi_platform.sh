...
###############################################################################
# build_oe4t_jetson_multi_platform_v31.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v31.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v31.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v31.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v31.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v31.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v31.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v31.sh
#
# Important:
...
#   - Bumps the Thor CUDA clean/repair stamp to v31 so this runs even if v28
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v31.sh thor cuda overlap guard
...
# parse error, so v31 uses only the renamed variable.
...
# END generated by build_oe4t_jetson_multi_platform_v31.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v31.sh.
BBPATH .= ":\${LAYERDIR}"
...
jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        ...
        sed -i \
            ...
            "${S}/Makefile.gnu"
        if ! grep -q 'jetson-builder force PIC for all shared-library builds' "${S}/Makefile.gnu"; then
            cat >> "${S}/Makefile.gnu" <<'EOF_MAKEFILE_PIC'

# jetson-builder force PIC for all shared-library builds
...
# v31 prunes both component files and sstate-control manifest lines. The latter
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v31.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v31 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA native state so the v31 nvcc/header collision fix is used"
...
clean_freeimage_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v31.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v31 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v31 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    ...
    log "Validating FreeImage v31 PIC bbappend is active"
    bitbake -e freeimage > /tmp/jetson-builder-freeimage-env.txt
    if ! grep -q 'jetsonbuilder31' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v31 bbappend is not active in BitBake metadata. Check bblayers.conf and meta-jetson-builder-fixes."
    fi
    if ! grep -q 'do_compile()' /tmp/jetson-builder-freeimage-env.txt || ! grep -q 'FREEIMAGE_PIC_CFLAGS' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v31 do_compile override is not visible in BitBake metadata."
    fi

    log "Forcing FreeImage compile validation with v31 PIC fix"
    bitbake -c cleansstate freeimage

...
###############################################################################
# build_oe4t_jetson_multi_platform_v46.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v46.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v46.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v46.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v46.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v46.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v46.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v46.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v46.sh --bundle
#
# Important:
...
#
# v46 notes:
#   - Keeps the v46 platform/build/bundle/e2fsprogs fixes intact.
...
#     stale partially-built Perl extension state from v46 is discarded.
#
# v44 notes:
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v46.sh
  ./build_oe4t_jetson_multi_platform_v46.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v46.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v46.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v46.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v46.sh.
PR:append = ".jetsonbuilder46"
...
jetson_builder_fix_opencv_cuda_native_include() {
    ...
    echo "jetson-builder v46 OpenCV CUDA native root: ${native_root}"
    echo "jetson-builder v46 OpenCV CUDA target root: ${target_root}"
    echo "jetson-builder v46 OpenCV CUDA selected target include: ${target_include:-none}"
    ls -ld "${native_root}" || true
    ls -ld "${native_include}" || true

    if [ ! -d "${native_include}" ]; then
        bbfatal "jetson-builder v46: ${native_include} still does not exist; cannot satisfy CMake CUDA imported target validation"
    fi
}

jetson_builder_sanitize_opencv_pkgconfig() {
    pc_file="${D}${libdir}/pkgconfig/opencv4.pc"
    [ -f "${pc_file}" ] || { echo "jetson-builder v46 OpenCV pkg-config sanitize: ${pc_file} not found; skipping"; return 0; }
    tmp_file="${pc_file}.jetsonbuilder46"
    ...
    if cmp -s "${pc_file}" "${tmp_file}"; then
        rm -f "${tmp_file}"
        echo "jetson-builder v46 OpenCV pkg-config sanitize: no changes needed for ${pc_file}"
    else
        mv "${tmp_file}" "${pc_file}"
        echo "jetson-builder v46 OpenCV pkg-config sanitize: sanitized ${pc_file}"
    fi
}
...
jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        sed -i \
            ...
            -e '/jetson-builder v46 force PIC/,+4d' \
            "${S}/Makefile.gnu"
        cat >> "${S}/Makefile.gnu" <<'EOF_MAKEFILE_PIC'

# jetson-builder v46 force PIC without replacing FreeImage include/macro flags
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v46.sh.
BBPATH .= ":\${LAYERDIR}"
...
#
# v46 only serialized perl-native. The uploaded log shows target perl itself
...
jetson_builder_e2fsprogs_validate_libext2fs() {
    for sym in \
        ...
        if ! jetson_builder_e2fsprogs_symbol_defined "${sym}"; then
            echo "jetson-builder v46 e2fsprogs: libext2fs candidates:" >&2
            ls -l "${B}/lib/ext2fs/libext2fs.so"* "${B}/lib/libext2fs.so"* 2>/dev/null || true
            bbfatal "jetson-builder v46: libext2fs.so is missing required symbol ${sym}; refusing to continue to e2fsck/do_install link"
        fi
    done
    bbnote "jetson-builder v46: libext2fs.so exports required e2fsck symbols"
}
...
jetson_builder_stage_libstdcxx_for_libcxx_native() {
    ...
    [ -n "${libstdcxx}" ] && [ -e "${libstdcxx}" ] || bbfatal "jetson-builder v46: could not locate host/native libstdc++.so for libcxx-native CMake sanity link"
    ...
    bbnote "jetson-builder v46: staged libcxx-native libstdc++ link: ${native_libdir}/libstdc++.so -> ${libstdcxx}"
}
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            for line in lines:
                ...
                kept.append(line)
            if changed:
                tmp_manifest = manifest + '.jetsonbuilder46.tmp'
...
ensure_active_freeimage_pic_fix() {
    ...
    local env_file="/tmp/jetson-builder-freeimage-v46-env.txt"
    if ! bitbake -e freeimage > "$env_file" 2>/tmp/jetson-builder-freeimage-v46-env.err; then
    ...
    if [ -n "$recipe_file" ] && [ -f "$recipe_file" ]; then
        ...
        log "Installing FreeImage v46 PIC bbappend next to active recipe: $recipe_file"
...
for manifest in manifest_candidates:
    ...
    for line in lines:
        ...
        kept.append(line)
    if changed:
        tmp_manifest = manifest.with_suffix(manifest.suffix + '.jetsonbuilder46.tmp')
...
clean_recipe_state_if_fix_changed() {
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "$stamp_name cleansstate guard already applied for the current v46 fix."
        return 0
    fi
...
clean_libcxx_native_state_if_needed() {
    ...
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "libcxx-native-stdcxx-clean-v46" "libcxx-native compiler-rt" \
...
clean_e2fsprogs_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "e2fsprogs-libext2fs-clean-v46" "e2fsprogs" \
...
clean_perl_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "perl-target-parallel-clean-v46" "perl perl-native" \
        "$fix_layer/recipes-devtools/perl/perl_%.bbappend"
}

validate_perl_parallel_fix() {
    log "Validating perl v46 target parallelism guard is visible"
    ...
    die "perl is active, but the v46 PARALLEL_MAKE=-j1 guard is not visible in BitBake metadata."
}
...
clean_thor_cuda_native_state_if_needed() {
    ...
    clean_recipe_state_if_fix_changed "$build_abs_dir" "thor-cuda-native-clean-v46" "cuda-nvcc-native cuda-nvcc-headers-native cuda-compiler-native" \
...
clean_freeimage_state_if_needed() {
    local platform="$1" distro_dir="$2"
    [ "$platform" = "thor" ] || return 0
    if ! bitbake -e freeimage >/tmp/jetson-builder-freeimage-v46-clean-env.txt 2>/dev/null; then
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v46.sha256"
    mkdir -p "$stamp_dir"
    local current_hash
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v46 fix."
        return 0
    fi
    log "Cleaning stale FreeImage state so the v46 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    local platform="$1"
    [ "$platform" = "thor" ] || return 0
    log "Validating FreeImage v46 PIC fix only if freeimage is active"
    ...
    grep -q 'jetson_builder_force_freeimage_pic' /tmp/jetson-builder-freeimage-env.txt || die "FreeImage is active, but the v46 PIC bbappend hook is not visible in BitBake metadata."
    log "Forcing FreeImage compile validation with v46 PIC fix"
    bitbake -c cleansstate freeimage
...
validate_opencv_cuda_sysroot_fix() {
    local platform="$1"
    [ "$platform" = "thor" ] || return 0
    log "Validating OpenCV v46 CUDA native include sysroot fix if opencv is active"
    ...
    grep -q 'jetson_builder_fix_opencv_cuda_native_include' /tmp/jetson-builder-opencv-env.txt || die "OpenCV is active, but the v46 CUDA sysroot bbappend hook is not visible in BitBake metadata."
    grep -q 'jetson_builder_sanitize_opencv_pkgconfig' /tmp/jetson-builder-opencv-env.txt || die "OpenCV is active, but the v46 OpenCV pkg-config sanitizer hook is not visible in BitBake metadata."
...
validate_e2fsprogs_libext2fs_fix() {
    local platform="$1"
    [ "$platform" = "thor" ] || return 0
    log "Validating e2fsprogs v46 libext2fs guard is visible"
    ...
    grep -q 'jetson_builder_e2fsprogs_validate_libext2fs' /tmp/jetson-builder-e2fsprogs-env.txt || die "e2fsprogs is active, but the v46 libext2fs validation hook is not visible in BitBake metadata."
}

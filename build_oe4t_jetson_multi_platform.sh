...
###############################################################################
# build_oe4t_jetson_multi_platform_v52.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v52.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v52.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v52.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v52.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v52.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v52.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v52.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v52.sh --bundle
#
# Important:
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v52.sh
  ./build_oe4t_jetson_multi_platform_v52.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v52.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v52.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v52.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v52.sh.
PR:append = ".jetsonbuilder51"
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v52.sh.
BBPATH .= ":\${LAYERDIR}"
...
# Makefile.  Normalizing source timestamps before configure prevents generated
# Makefiles from appearing stale during compile.  Keep Perl extension builds
# serialized as an additional narrow guard.
...
jetson_builder_perl_fix_source_time_skew() {
    for tree in "${S}" "${B}"; do
        [ -d "${tree}" ] || continue
        find "${tree}" -exec touch -h -d '2000-01-01 00:00:00 UTC' {} + 2>/dev/null ||             find "${tree}" -exec touch -d '2000-01-01 00:00:00 UTC' {} +
    done
}
...
clean_perl_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "perl-timestamp-skew-clean-v52" "perl perl-native" \
...
clean_libidn2_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "libidn2-unistring-install-clean-v52" "libidn2" \
        "$fix_layer/recipes-support/libidn2/libidn2_%.bbappend"
}

validate_libidn2_unistring_fix() {
    log "Validating libidn2 v52 unistring install guard is visible"
    ...
    grep -q 'jetson_builder_libidn2_sanitize_unistring_makefiles' /tmp/jetson-builder-libidn2-env.txt || die "libidn2 is active, but the v52 unistring Makefile sanitizer hook is not visible in BitBake metadata."
}

validate_perl_parallel_fix() {
    log "Validating perl v52 timestamp-skew and parallelism guard is visible"
    ...
    if ! grep -q 'jetson_builder_perl_fix_source_time_skew' /tmp/jetson-builder-perl-env.txt; then
        die "perl is active, but the v52 timestamp-skew guard is not visible in BitBake metadata."
    fi
    if grep -Eq '^PARALLEL_MAKE="-j1"|^PARALLEL_MAKE='-j1'' /tmp/jetson-builder-perl-env.txt; then
        return 0
    fi
    die "perl is active, but the v52 PARALLEL_MAKE=-j1 guard is not visible in BitBake metadata."
}

...
###############################################################################
# build_oe4t_jetson_multi_platform_v53.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v53.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v53.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v53.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v53.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v53.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v53.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v53.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v53.sh --bundle
#
# Important:
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v53.sh
  ./build_oe4t_jetson_multi_platform_v53.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v53.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v53.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v53.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v53.sh.
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v53.sh.
BBPATH .= ":\${LAYERDIR}"
...
clean_perl_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "perl-timestamp-skew-clean-v53" "perl perl-native" \
...
validate_perl_parallel_fix() {
    log "Validating perl v53 timestamp-skew and parallelism guard is visible"
    ...
    if ! grep -q 'jetson_builder_perl_fix_source_time_skew' /tmp/jetson-builder-perl-env.txt; then
        die "perl is active, but the v53 timestamp-skew guard is not visible in BitBake metadata."
    fi
    if grep -Eq '^PARALLEL_MAKE="-j1"|^PARALLEL_MAKE='-j1'' /tmp/jetson-builder-perl-env.txt; then
        return 0
    fi
    die "perl is active, but the v53 PARALLEL_MAKE=-j1 guard is not visible in BitBake metadata."
}

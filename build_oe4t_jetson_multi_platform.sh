...
###############################################################################
# build_oe4t_jetson_multi_platform_v56.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v56.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v56.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v56.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v56.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v56.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v56.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v56.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v56.sh --bundle
#
# Important:
...
#     directories are present. v56 repairs the actual CMake source root first
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v56.sh
  ./build_oe4t_jetson_multi_platform_v56.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v56.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v56.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v56.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v56.sh.
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v56.sh.
BBPATH .= ":\${LAYERDIR}"
...
jetson_builder_compiler_rt_repair_missing_runtimes_source() {
    ...
    # v56: Repair the source tree CMake is actually about to configure first.
    # v55 looked at LLVM_PROJECT_SOURCE_DIR before OECMAKE_SOURCEPATH/S, which
    # can point at the unsuffixed llvm-project-source work-shared tree while
    # compiler-rt-native configures from a PR-suffixed tree such as
    # llvm-project-source-...-r1.jetsonbuilder52. That leaves CMake pointed at
    # a missing runtimes directory even though another tree was repaired.
    candidate_roots=""

    for maybe_source in         "${OECMAKE_SOURCEPATH:-}"         "${S:-}"; do
        if [ -n "$maybe_source" ]; then
            case "$maybe_source" in
                */runtimes) maybe_root="${maybe_source%/runtimes}" ;;
...
"
    ...
    for maybe_root in         "${LLVM_PROJECT_SOURCE_DIR:-}"         "${LLVM_PROJECT_SOURCE:-}"         "${LLVM_SOURCE_DIR:-}"; do
        if [ -n "$maybe_root" ] && [ -d "$maybe_root" ]; then
            candidate_roots="${candidate_roots}${maybe_root}
"
        fi
    done

    if [ -n "${TMPDIR:-}" ] && [ -d "${TMPDIR}/work-shared" ]; then
        found_shared_roots="$(find "${TMPDIR}/work-shared" -maxdepth 4 -type d -name 'llvm-project-*.src' -print 2>/dev/null || true)"
...
"
    ...
    if [ -z "$found_root" ] || [ ! -d "$found_root" ]; then
        bbnote "jetson-builder v56: no shared llvm-project source root found for ${PN}; leaving compiler-rt source path unchanged"
        return 0
    fi

    runtimes_dir="$found_root/runtimes"
    bbnote "jetson-builder v56: selected LLVM source root for ${PN}: $found_root"

    if [ -f "$runtimes_dir/CMakeLists.txt" ]; then
        bbnote "jetson-builder v56: LLVM runtimes source already exists for ${PN}: $runtimes_dir"
        return 0
    fi
    ...
    if [ -f "$found_root/llvm/runtimes/CMakeLists.txt" ]; then
        ...
        bbnote "jetson-builder v56: linked missing LLVM runtimes source for ${PN}: $runtimes_dir -> $found_root/llvm/runtimes"
        return 0
    fi

    compiler_rt_src=""
    for candidate in         "$found_root/compiler-rt"         "${S:-}/compiler-rt"         "${S:-}"; do
    ...
    if [ -n "$compiler_rt_src" ]; then
        ...
        bbnote "jetson-builder v56: created empty fallback LLVM runtimes entry point for ${PN} at $runtimes_dir; individual runtime sources were left linked for diagnostics"
    else
        bbwarn "jetson-builder v56: compiler-rt source was not found under $found_root; creating empty compiler-rt fallback for ${PN}"
    fi
...
add_custom_target(compiler_rt_empty ALL
    COMMAND ${CMAKE_COMMAND} -E echo "jetson-builder v56: empty compiler-rt fallback")
install(CODE "message(STATUS "jetson-builder v56: installing empty compiler-rt fallback")")
...
clean_compiler_rt_runtimes_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "compiler-rt-missing-runtimes-clean-v56" "compiler-rt compiler-rt-native" \
        "$fix_layer/recipes-devtools/clang/compiler-rt_%.bbappend"
}
...
validate_compiler_rt_runtimes_fix() {
    log "Validating compiler-rt v56 missing-runtimes configure guard is visible"
    ...
    grep -q 'jetson_builder_compiler_rt_repair_missing_runtimes_source' /tmp/jetson-builder-compiler-rt-native-env.txt || die "compiler-rt-native is active, but the v56 missing-runtimes configure guard is not visible in BitBake metadata."
    grep -q 'JetsonBuilderCompilerRTFallback' /tmp/jetson-builder-compiler-rt-native-env.txt || die "compiler-rt-native is active, but the v56 empty fallback CMake guard is not visible in BitBake metadata."
}

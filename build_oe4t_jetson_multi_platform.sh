...
###############################################################################
# build_oe4t_jetson_multi_platform_v30.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v30.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v30.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v30.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v30.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v30.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v30.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v30.sh
#
# Important:
...
#   - Bumps the Thor CUDA clean/repair stamp to v30 so this runs even if v28
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v30.sh thor cuda overlap guard
...
# parse error, so v30 uses only the renamed variable.
...
# END generated by build_oe4t_jetson_multi_platform_v30.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v30.sh.
BBPATH .= ":\${LAYERDIR}"
...
EOF_LINUX_FW
    ...
    # FreeImage 3.18.0's Makefile.gnu uses "CFLAGS ?= ... -fPIC" upstream,
    # but BitBake exports its own target CFLAGS, so the makefile default can be
    # bypassed. On aarch64 that leaves LibJXR objects non-PIC and the shared
    # link fails with R_AARCH64_ADR_PREL_PG_HI21 "recompile with -fPIC".
    cat > "$fix_layer/recipes-multimedia/freeimage/freeimage_%.bbappend" <<'EOF_FREEIMAGE'
# Force PIC into FreeImage/LibJXR target objects before libfreeimage.so is linked.
PR:append = ".jetsonbuilder30"

CFLAGS:append = " -fPIC"
CXXFLAGS:append = " -fPIC"
EXTRA_OEMAKE:append = " CFLAGS='${CFLAGS} -fPIC' CXXFLAGS='${CXXFLAGS} -fPIC'"

jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        # Do not rely on Makefile.gnu's CFLAGS ?= default: Yocto normally
        # supplies CFLAGS, so append PIC unconditionally at makefile level too.
        if ! grep -q 'jetson-builder force PIC for aarch64 shared link' "${S}/Makefile.gnu"; then
            cat >> "${S}/Makefile.gnu" <<'EOF_MAKEFILE_PIC'

# jetson-builder force PIC for aarch64 shared link
...
# v30 prunes both component files and sstate-control manifest lines. The latter
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v30.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v30 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA native state so the v30 nvcc/header collision fix is used"
...
clean_freeimage_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v30.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v30 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v30 -fPIC fix is used"

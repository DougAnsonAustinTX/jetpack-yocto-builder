...
###############################################################################
# build_oe4t_jetson_multi_platform_v26.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v26.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v26.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v26.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v26.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v26.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v26.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v26.sh
#
# Important:
...
#
# v26 fixes:
#   - Adds a cuda-compiler bbappend prefunc that prunes stale cuda-nvcc-native
#     sysroot component headers immediately before cuda-compiler-native prepares
#     its recipe sysroot. This targets the exact task that reports the
#     fatbinary_section.h overlap.
#   - Adds a Thor-only SSTATE_ALLOW_OVERLAP_FILES fallback for CUDA header
#     paths in local.conf, so an unavoidable duplicate header can no longer halt
#     do_prepare_recipe_sysroot.
#   - Repairs any already-materialized cuda-nvcc-native sysroot component on
#     every Thor run, not only after a cleansstate stamp change.
#
# v25 fixes:
#   - Hardens the Thor/CUDA 13 nvcc/header collision fix so it removes CUDA
#     headers using globbed cuda-* roots instead of relying only on CUDA_VERSION.
#   - Adds a native sysroot preprocess guard that strips cuda-nvcc-native headers
#     immediately before sysroot staging, keeping cuda-nvcc-headers-native as the
#     sole owner of fatbinary_section.h.
#   - Forces a cleansstate of cuda-nvcc-native, cuda-nvcc-headers-native, and
#     cuda-compiler-native for Thor once the local fix layer has been generated,
#     so stale pre-v25 sysroot manifests cannot keep causing the same collision.
#
# v24 fixes:
#   - Adds a Thor/CUDA 13 local cuda-nvcc bbappend that removes staged CUDA
#     headers from cuda-nvcc-native, including the CUDA 13 target include path
#     /usr/local/cuda-13.0/targets/*/include. Those headers are provided by
#     cuda-nvcc-headers-native; without this, BitBake aborts during
#     cuda-compiler-native do_prepare_recipe_sysroot because fatbinary_section.h
#     is installed by both cuda-nvcc-native and cuda-nvcc-headers-native.
#
# v23 fixes:
#   - Adds a final generated-bbappend validation pass for every platform, including
#     orin-nx, so stale or branch-inapplicable local bbappends are removed before
#     BitBake parsing. This is intentionally no-op for the working Orin Super Nano
#     master branch where all matching recipes exist.
#
# v22 fixes:
#   - Keeps the working Orin Super Nano behavior intact while avoiding a Thor
#     parse failure from a dangling tegra-flashtools-native_%.bbappend. The
#     Thor branch does not provide a matching tegra-flashtools-native recipe,
#     so the generated local fix layer now creates that bbappend only when the
#     checked-out OE4T branch actually contains a matching recipe. Stale copies
#     are removed automatically for branches that do not need it.
#
# v20 fixes:
...
append_once_block() {
    ...
    # Remove Thor CUDA guard blocks from prior v26 runs before appending the
    # current one. The main generated block keeps the historical v15 marker so
    # older script output is replaced in-place.
    sed -i '/^# BEGIN generated by build_oe4t_jetson_multi_platform_v26.sh thor cuda overlap guard$/,/^# END generated by build_oe4t_jetson_multi_platform_v26.sh thor cuda overlap guard$/d' "$file"
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v26.sh thor cuda overlap guard

# Thor / CUDA 13: cuda-nvcc-native and cuda-nvcc-headers-native can both
# expose the same CUDA header paths while cuda-compiler-native prepares its
# recipe sysroot. The recipe bbappends in meta-jetson-builder-fixes remove the
# duplicate headers from cuda-nvcc-native; this is the last-resort staging
# whitelist for any identical overlap that still reaches sstate/sysroot code.
...
# END generated by build_oe4t_jetson_multi_platform_v26.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v26.sh.
...
EOF_TEGRA_LIBS
    ...
    local cuda_nvcc_recipe_glob
    local has_cuda_nvcc_recipe
    cuda_nvcc_recipe_glob="$distro_dir/layers/meta-tegra/recipes-devtools/cuda/cuda-nvcc_"*.bb
    has_cuda_nvcc_recipe=0
...
#
# cuda-nvcc-headers owns the CUDA header payload. CUDA 13 packages also place
# headers under targets/<arch>/include, and cuda-nvcc-native can otherwise stage
# those headers too. That makes cuda-compiler-native fail while preparing its
# recipe sysroot with an error like:
#   fatbinary_section.h is installed by both cuda-nvcc-native and cuda-nvcc-headers-native
#
# Keep the fix scoped to cuda-nvcc so cuda-nvcc-headers remains the single owner
# of the headers. This is intentionally applied at both install time and native
# sysroot-preprocess time so stale or recipe-layout-specific header paths do not
# leak into the cuda-nvcc-native sysroot component.
PR:append = ".jetsonbuilder26"
...
#
# The observed failure happens in cuda-compiler-native:do_prepare_recipe_sysroot
# when BitBake is assembling that recipe's dependency sysroot. If an older or
# upstream sstate component for cuda-nvcc-native already contains CUDA headers,
# cleaning cuda-nvcc's ${D} during do_install is too early to help. This prefunc
# removes the duplicate header payload from the materialized cuda-nvcc-native
# sysroot component immediately before cuda-compiler-native prepares its sysroot.
PR:append = ".jetsonbuilder26"
...
python jetson_builder_prune_cuda_nvcc_native_component () {
    ...
    if removed:
        bb.warn('jetson-builder CUDA fix: pruned cuda-nvcc-native duplicate header dirs before do_prepare_recipe_sysroot: %s' % ', '.join(removed))
}
...
EOF_CUDA_COMPILER
        ...
        # Remove stale CUDA workaround copies from non-Thor workspaces or from
        # branches that do not have cuda-nvcc as a recipe.
    ...
    local tegra_flashtools_recipe_glob
    local has_tegra_flashtools_native
    tegra_flashtools_recipe_glob="$distro_dir/layers/meta-tegra/recipes-bsp/tegra-binaries/tegra-flashtools-native_"*.bb
    has_tegra_flashtools_native=0
...
EOF_TEGRA_FLASHTOOLS
        ...
        # Thor / JetPack 7 branches may not provide tegra-flashtools-native.
        # Leaving a wildcard bbappend behind would make BitBake fail with:
        #   No recipes in default available for: tegra-flashtools-native_%.bbappend
        # Remove any stale copies from earlier script versions in this workspace.
    ...
    # Safety net for all platforms, including orin-nx: do not leave generated
    # bbappends in this local layer unless the checked-out source tree has at
    # least one matching recipe. This prevents BitBake's dangling append error:
    #   No recipes in default available for: <name>.bbappend
    # It is deliberately a no-op for the known-good Orin Super Nano master
    # workspace because the matching recipes are present there.
...
repair_thor_cuda_sysroot_components() {
    ...
    components_dir="$build_abs_dir/tmp/sysroots-components"
    [ -d "$components_dir" ] || return 0

    log "Repairing any already-materialized cuda-nvcc-native CUDA header sysroot component"

    find "$components_dir" -path '*/cuda-nvcc-native/usr/local/cuda-*/include' -type d -prune -exec rm -rf {} + 2>/dev/null || true
    find "$components_dir" -path '*/cuda-nvcc-native/usr/local/cuda-*/targets/*/include' -type d -prune -exec rm -rf {} + 2>/dev/null || true
}
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local cuda_fix_bbappend
    cuda_fix_bbappend="$fix_layer/recipes-devtools/cuda/cuda-nvcc_%.bbappend"

    if [ ! -f "$cuda_fix_bbappend" ]; then
        warn "Thor CUDA collision fix bbappend was not found at $cuda_fix_bbappend; skipping CUDA cleansstate guard."
        return 0
    fi
    ...
    local stamp_file
    local current_hash
    stamp_dir="$build_abs_dir/conf/.jetson-builder-stamps"
    stamp_file="$stamp_dir/thor-cuda-native-clean-v26.sha256"
    mkdir -p "$stamp_dir"

    local cuda_compiler_fix_bbappend
    cuda_compiler_fix_bbappend="$fix_layer/recipes-devtools/cuda/cuda-compiler_%.bbappend"

    if [ -f "$cuda_compiler_fix_bbappend" ]; then
        current_hash="$(cat "$cuda_fix_bbappend" "$cuda_compiler_fix_bbappend" | sha256sum | awk '{print $1}')"
    else
        current_hash="$(sha256sum "$cuda_fix_bbappend" | awk '{print $1}')"
    fi

    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v26 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA native state so the nvcc/header collision fix is used"
    bitbake -c cleansstate cuda-nvcc-native cuda-nvcc-headers-native cuda-compiler-native

    # BitBake cleansstate should remove the relevant manifests and sysroot
    # components. Remove any leftover component directories defensively so an
    # interrupted earlier build cannot keep an old fatbinary_section.h owner.
...
EOF_BUILD_CONFIG
    ...
    clean_thor_cuda_native_state_if_needed "$platform" "$build_abs_dir" "$workspace/tegra-demo-distro/layers/meta-jetson-builder-fixes"

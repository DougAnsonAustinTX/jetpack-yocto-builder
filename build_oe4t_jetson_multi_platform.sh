...
###############################################################################
# build_oe4t_jetson_multi_platform_v25.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v25.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v25.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v25.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v25.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v25.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v25.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v25.sh
#
# Important:
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v25.sh.
...
clean_thor_cuda_native_state_if_needed() {
    ...
    stamp_file="$stamp_dir/thor-cuda-native-clean-v25.sha256"
    mkdir -p "$stamp_dir"

    current_hash="$(sha256sum "$cuda_fix_bbappend" | awk '{print $1}')"

    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v25 fix."
        return 0
    fi

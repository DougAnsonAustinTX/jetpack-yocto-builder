...
###############################################################################
# build_oe4t_jetson_multi_platform_v27.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v27.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v27.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v27.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v27.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v27.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v27.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v27.sh
#
# Important:
...
#
# v27 fixes:
#   - Keeps the v26 CUDA file pruning, but also edits/removes stale
#     sstate-control manifest ownership records for cuda-nvcc-native header
#     paths before cuda-compiler-native prepares its recipe sysroot.
#   - Adds both the current SSTATE_ALLOW_OVERLAP_FILES and legacy
#     SSTATE_DUPWHITELIST names for Thor CUDA header overlaps, with exact known
#     CUDA 13 fatbinary_section.h paths instead of relying only on wildcards.
#   - Bumps the Thor CUDA cleansstate stamp to v27 and includes local.conf in
#     the hash so this fix is forced even if v26 already ran.
###############################################################################
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v27.sh thor cuda overlap guard
...
#
# v27 keeps both variable names because older branches used SSTATE_DUPWHITELIST
# and newer branches use SSTATE_ALLOW_OVERLAP_FILES.
...
SSTATE_DUPWHITELIST += " /usr/local/cuda-13.0/targets/sbsa-linux/include/fatbinary_section.h"
SSTATE_DUPWHITELIST += " /usr/local/cuda-13.0/include/* /usr/local/cuda-13.0/targets/*/include/* /usr/local/cuda-*/include/* /usr/local/cuda-*/targets/*/include/*"

# END generated by build_oe4t_jetson_multi_platform_v27.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v27.sh.
BBPATH .= ":\${LAYERDIR}"
...
# v27 prunes both component files and sstate-control manifest lines. The latter
...
python jetson_builder_prune_cuda_nvcc_native_component () {
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            if changed:
                tmp_manifest = manifest + '.jetsonbuilder27.tmp'
...
repair_thor_cuda_sysroot_components() {
    ...
    local components_dir="$build_abs_dir/tmp/sysroots-components"
    local sstate_control_dir="$build_abs_dir/tmp/sstate-control"

    if [ -d "$components_dir" ]; then
        log "Repairing already-materialized cuda-nvcc-native CUDA header sysroot component"
    ...
    if [ -d "$sstate_control_dir" ]; then
        log "Repairing stale cuda-nvcc-native sstate manifest ownership entries for CUDA headers"
        python3 - "$sstate_control_dir" <<'PY_REPAIR_MANIFESTS'
from pathlib import Path
import sys
root = Path(sys.argv[1])
for manifest in root.glob('manifest-*-cuda-nvcc-native.populate_sysroot'):
    try:
        lines = manifest.read_text(errors='ignore').splitlines(True)
    except OSError:
        continue
    kept = []
    changed = False
    for line in lines:
        s = line.strip()
        if '/usr/local/cuda-' in s and '/include' in s:
            changed = True
            continue
        kept.append(line)
    if changed:
        manifest.write_text(''.join(kept))
        print(f'  pruned {manifest}')
PY_REPAIR_MANIFESTS
    fi
}
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v27.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v27 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA native state so the v27 nvcc/header collision fix is used"

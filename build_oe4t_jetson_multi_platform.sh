...
###############################################################################
# build_oe4t_jetson_multi_platform_v55.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v55.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v55.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v55.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v55.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v55.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v55.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v55.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v55.sh --bundle
#
# Important:
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v55.sh
  ./build_oe4t_jetson_multi_platform_v55.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v55.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v55.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v55.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v55.sh.
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v55.sh.
BBPATH .= ":\${LAYERDIR}"
...
jetson_builder_compiler_rt_repair_missing_runtimes_source() {
    ...
    found_root=""
    if [ -n "$LLVM_PROJECT_SOURCE_DIR" ] && [ -d "$LLVM_PROJECT_SOURCE_DIR" ]; then
        found_root="$LLVM_PROJECT_SOURCE_DIR"
    elif [ -n "$LLVM_PROJECT_SOURCE" ] && [ -d "$LLVM_PROJECT_SOURCE" ]; then
        found_root="$LLVM_PROJECT_SOURCE"
    elif [ -n "$LLVM_SOURCE_DIR" ] && [ -d "$LLVM_SOURCE_DIR" ]; then
        found_root="$LLVM_SOURCE_DIR"
    elif [ -n "${TMPDIR}" ] && [ -d "${TMPDIR}/work-shared" ]; then
        found_root="$(find "${TMPDIR}/work-shared" -maxdepth 4 -type d -name 'llvm-project-*.src' -print -quit 2>/dev/null || true)"
    fi

    if [ -z "$found_root" ] || [ ! -d "$found_root" ]; then
        bbnote "jetson-builder v55: no shared llvm-project source root found for ${PN}; leaving compiler-rt source path unchanged"
        return 0
    fi

    runtimes_dir="$found_root/runtimes"
    if [ -f "$runtimes_dir/CMakeLists.txt" ]; then
        bbnote "jetson-builder v55: LLVM runtimes source already exists for ${PN}: $runtimes_dir"
        return 0
    fi
    ...
    if [ -f "$found_root/llvm/runtimes/CMakeLists.txt" ]; then
        ...
        bbnote "jetson-builder v55: linked missing LLVM runtimes source for ${PN}: $runtimes_dir -> $found_root/llvm/runtimes"
        return 0
    fi

    compiler_rt_src=""
    for candidate in \
        "$found_root/compiler-rt" \
        "${S}/compiler-rt" \
        "${S}"; do
        if [ -f "$candidate/CMakeLists.txt" ] && [ -d "$candidate/lib" ]; then
    ...
    if [ -n "$compiler_rt_src" ]; then
        ...
        bbnote "jetson-builder v55: created empty fallback LLVM runtimes entry point for ${PN} at $runtimes_dir; individual runtime sources were left linked for diagnostics"
    else
        bbwarn "jetson-builder v55: compiler-rt source was not found under $found_root; creating empty compiler-rt fallback for ${PN}"
    fi
...
add_custom_target(compiler_rt_empty ALL
    COMMAND ${CMAKE_COMMAND} -E echo "jetson-builder v55: empty compiler-rt fallback")
install(CODE "message(STATUS \"jetson-builder v55: installing empty compiler-rt fallback\")")
...
clean_compiler_rt_runtimes_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "compiler-rt-missing-runtimes-clean-v55" "compiler-rt compiler-rt-native" \
        "$fix_layer/recipes-devtools/clang/compiler-rt_%.bbappend"
}
...
validate_compiler_rt_runtimes_fix() {
    log "Validating compiler-rt v55 missing-runtimes configure guard is visible"
    ...
    grep -q 'jetson_builder_compiler_rt_repair_missing_runtimes_source' /tmp/jetson-builder-compiler-rt-native-env.txt || die "compiler-rt-native is active, but the v55 missing-runtimes configure guard is not visible in BitBake metadata."
    grep -q 'JetsonBuilderCompilerRTFallback' /tmp/jetson-builder-compiler-rt-native-env.txt || die "compiler-rt-native is active, but the v55 empty fallback CMake guard is not visible in BitBake metadata."
}

...
###############################################################################
# build_oe4t_jetson_multi_platform_v32.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v32.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v32.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v32.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v32.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v32.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v32.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v32.sh
#
# Important:
...
#   - Bumps the Thor CUDA clean/repair stamp to v32 so this runs even if v28
#     already created stamps.
#   - Adds a FreeImage 3.18.0 bbappend that forces -fPIC into C/C++ object
#     compilation and cleans stale non-PIC objects/state. This fixes the
#     aarch64 LibJXR/JXRGlue.o shared-link relocation failure.
###############################################################################
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v32.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v32.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v32.sh.
BBPATH .= ":\${LAYERDIR}"
...
EOF_LINUX_FW
    ...
    # FreeImage 3.18.0's Makefile.gnu defaults include -fPIC, but the recipe
    # invokes make with Yocto-exported CFLAGS/CXXFLAGS, which bypasses those
    # defaults. On aarch64 GCC 15/binutils then reject non-PIC bundled objects
    # when linking libfreeimage.so. Override do_compile so PIC is passed on the
    # actual make command line, and patch Makefile.gnu as a second line of
    # defense for all bundled C and C++ objects (LibJXR, OpenEXR, etc.).
    cat > "$fix_layer/recipes-multimedia/freeimage/freeimage_%.bbappend" <<'EOF_FREEIMAGE'
# Force PIC into every FreeImage bundled object before libfreeimage.so is linked.
# This must affect the actual make invocation, not only BitBake metadata.
PR:append = ".jetsonbuilder32"
...
jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        # Make the upstream makefile robust even when CFLAGS/CXXFLAGS are
        ...
        sed -i \
            ...
            "${S}/Makefile.gnu"
        if ! grep -q 'jetson-builder v32 force PIC for all shared-library builds' "${S}/Makefile.gnu"; then
            cat >> "${S}/Makefile.gnu" <<'EOF_MAKEFILE_PIC'

# jetson-builder v32 force PIC for all shared-library builds
...
EOF_MAKEFILE_PIC
    ...
    # Remove any non-PIC objects left behind by an interrupted/failed compile.
...
# The stock recipe's compile log showed make being called as:
#   make -C ${S} -f Makefile.gnu
# with no command-line CFLAGS/CXXFLAGS and no -fPIC in the compile commands.
# Override the task so the command line itself contains PIC flags.
do_compile() {
    jetson_builder_force_freeimage_pic
...
install_freeimage_pic_bbappend_into_active_recipe_layer() {
    local platform="$1"
    local distro_dir="$2"
    [ "$platform" = "thor" ] || return 0

    log "Installing FreeImage v32 PIC bbappend next to the active recipe"
    ...
    while IFS= read -r -d '' recipe; do
        ...
        recipe_base="$(basename "$recipe" .bb)"
        append_path="$recipe_dir/${recipe_base}.bbappend"

        cat > "$append_path" <<'EOF_FREEIMAGE_ACTIVE'
# Generated by build_oe4t_jetson_multi_platform_v32.sh.
# Force PIC into every FreeImage bundled object before libfreeimage.so is linked.
# This copy lives beside the active upstream recipe so BitBake parses it even if
# the generated local fixes layer is ignored or has only bbappends.
PR:append = ".jetsonbuilder32"
...
EOF_FREEIMAGE_ACTIVE
        log "Installed FreeImage PIC bbappend: $append_path"
    done < <(find "$distro_dir/layers" -path '*/recipes-multimedia/freeimage/freeimage_*.bb' -type f -print0)

    [ "$found" = "1" ] || die "Could not find an active freeimage_*.bb recipe under $distro_dir/layers."
}
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v32.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v32 fix."
        return 0
    fi
...
clean_freeimage_state_if_needed() {
    ...
    local distro_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    local freeimage_fix_bbappend="$fix_layer/recipes-multimedia/freeimage/freeimage_%.bbappend"
    local stamp_dir="$distro_dir/conf/.jetson-builder-stamps"
    local stamp_file="$stamp_dir/freeimage-pic-clean-v32.sha256"
    mkdir -p "$stamp_dir"

    [ -f "$freeimage_fix_bbappend" ] || return 0

    local current_hash
    current_hash="$(sha256sum "$freeimage_fix_bbappend" | awk '{print $1}')"
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v32 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v32 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    ...
    log "Validating FreeImage v32 PIC bbappend is active"
    bitbake -e freeimage > /tmp/jetson-builder-freeimage-env.txt
    if ! grep -q 'jetsonbuilder32' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v32 bbappend is not active in BitBake metadata. Check bblayers.conf and meta-jetson-builder-fixes."
    fi
    if ! grep -q 'do_compile()' /tmp/jetson-builder-freeimage-env.txt || ! grep -q 'FREEIMAGE_PIC_CFLAGS' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v32 do_compile override is not visible in BitBake metadata."
    fi

    log "Forcing FreeImage compile validation with v32 PIC fix"
    bitbake -c cleansstate freeimage
...
build_one_platform() {
    ...
    install_freeimage_pic_bbappend_into_active_recipe_layer "$platform" "$workspace/tegra-demo-distro"

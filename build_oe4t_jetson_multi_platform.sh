...
###############################################################################
# build_oe4t_jetson_multi_platform_v38.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v38.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v38.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v38.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v38.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v38.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v38.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v38.sh
#
# Important:
...
#
# v38 fixes:
#   - Carries forward the v35/v37 Thor CUDA duplicate-header producer/manifest
#     fix and OpenCV CUDA native include sysroot repair.
#   - Adds an OpenCV opencv4.pc sanitizer so opencv-dev no longer fails
#     do_package_qa with TMPDIR references in /usr/lib/pkgconfig/opencv4.pc
#     [buildpaths]. This fixes the generated metadata instead of disabling QA.
#   - Selects avahi-libnss-mdns as the preferred libnss-mdns runtime provider
#     to remove the BitBake provider ambiguity warning.
#   - Bumps all Thor CUDA/OpenCV/FreeImage stamps and PR suffixes to v38 so the
#     repairs run even if an older version already stamped the tree.
###############################################################################
...
# Select the mDNS NSS provider explicitly to avoid runtime provider ambiguity.
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v38.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v38.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v38.sh.
#
# OpenCV Thor fixes:
#   1. Carry forward the CUDA native include sysroot repair from v37.
#      OpenCV is a target recipe, but CMake's FindCUDAToolkit can derive
#      imported CUDA target include directories from native nvcc. On Thor CUDA
#      13 this can produce CUDA::cudart/CUDA::cuda_driver with:
#        ${RECIPE_SYSROOT_NATIVE}/usr/local/cuda-13.0/include
#      even when the real headers are in ${RECIPE_SYSROOT}. CMake validates
#      imported INTERFACE_INCLUDE_DIRECTORIES during generation, so the path
#      must exist.
#
#   2. New in v38: sanitize ${D}${libdir}/pkgconfig/opencv4.pc during
#      do_install so opencv-dev does not fail do_package_qa with:
#        contains reference to TMPDIR [buildpaths]
#      This fixes the bad metadata rather than disabling buildpaths QA.
PR:append = ".jetsonbuilder38"
...
jetson_builder_fix_opencv_cuda_native_include() {
    ...
    echo "jetson-builder v38 OpenCV CUDA native root: ${native_root}"
    echo "jetson-builder v38 OpenCV CUDA target root: ${target_root}"
    echo "jetson-builder v38 OpenCV CUDA selected target include: ${target_include:-none}"
    ls -ld "${native_root}" || true
    ls -ld "${native_include}" || true

    if [ ! -d "${native_include}" ]; then
        bbfatal "jetson-builder v38: ${native_include} still does not exist; cannot satisfy CMake CUDA imported target validation"
    fi
}
...
python do_install:append() {
    ...
    dvar = d.getVar('D') or ''
    libdir = d.getVar('libdir') or '/usr/lib'
    pc_path = os.path.join(dvar, libdir.lstrip('/'), 'pkgconfig', 'opencv4.pc')

    if not os.path.exists(pc_path):
        bb.note('jetson-builder v38 OpenCV pkg-config sanitize: opencv4.pc not found at %s' % pc_path)
        return
    ...
    bad_roots = [p for p in bad_roots if p]

    for root in bad_roots:
        escaped = re.escape(root)
        # Drop whitespace-separated compiler/linker tokens containing build paths.
        text = re.sub(r'(^|[ \t])[^ \t\r\n]*' + escaped + r'[^ \t\r\n]*', r'\1', text)

    # Normalize standard pkg-config variables to target-side paths.
    text = re.sub(r'^prefix=.*$', 'prefix=/usr', text, flags=re.MULTILINE)
    text = re.sub(r'^exec_prefix=.*$', 'exec_prefix=${prefix}', text, flags=re.MULTILINE)
    text = re.sub(r'^libdir=.*$', 'libdir=${exec_prefix}/lib', text, flags=re.MULTILINE)
    text = re.sub(r'^includedir=.*$', 'includedir=${prefix}/include/opencv4', text, flags=re.MULTILINE)

    cleaned_lines = []
    for line in text.splitlines():
        if line.startswith(('Libs:', 'Libs.private:', 'Cflags:', 'Cflags.private:')):
    ...
    if text != original:
        with open(pc_path, 'w', encoding='utf-8') as f:
            f.write(text)
        bb.note('jetson-builder v38 OpenCV pkg-config sanitize: sanitized build paths from %s' % pc_path)
    else:
        bb.note('jetson-builder v38 OpenCV pkg-config sanitize: no build paths found in %s' % pc_path)
}
EOF_OPENCV_CUDA
}
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v38.sh.
BBPATH .= ":\${LAYERDIR}"
...
# v38 is intentionally surgical: remove only the known duplicate header from
...
# v38 removes only the exact duplicate fatbinary_section.h ownership entry from
...
python jetson_builder_prune_cuda_nvcc_native_component () {
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            if changed:
                tmp_manifest = manifest + '.jetsonbuilder38.tmp'
...
jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        sed -i \
            ...
            -e '/jetson-builder v38 force PIC/,+4d' \
            "${S}/Makefile.gnu"

        cat >> "${S}/Makefile.gnu" <<'EOF_MAKEFILE_PIC'

# jetson-builder v38 force PIC without replacing FreeImage include/macro flags
...
ensure_active_freeimage_pic_fix() {
    ...
    local env_file="/tmp/jetson-builder-freeimage-v38-env.txt"
    if ! bitbake -e freeimage > "$env_file" 2>/tmp/jetson-builder-freeimage-v38-env.err; then
    ...
    if [ -n "$recipe_file" ] && [ -f "$recipe_file" ]; then
        ...
        log "Installing FreeImage v38 PIC bbappend next to active recipe: $recipe_file"
...
for manifest in manifest_candidates:
    ...
    for line in lines:
        ...
        kept.append(line)
    if changed:
        tmp_manifest = manifest.with_suffix(manifest.suffix + '.jetsonbuilder38.tmp')
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v38.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v38 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA/OpenCV state so the v38 CUDA include repair is used"
...
clean_freeimage_state_if_needed() {
    ...
    if ! bitbake -e freeimage >/tmp/jetson-builder-freeimage-v38-clean-env.txt 2>/dev/null; then
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v38.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v38 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v38 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    ...
    log "Validating FreeImage v38 PIC fix only if freeimage is active"
    ...
    if ! grep -q 'jetsonbuilder38' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage is active, but the v38 PIC bbappend is not active in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_force_freeimage_pic' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v38 PIC patch hook is not visible in BitBake metadata."
    fi

    log "Forcing FreeImage compile validation with v38 PIC fix"
    bitbake -c cleansstate freeimage
...
validate_opencv_cuda_sysroot_fix() {
    ...
    log "Validating OpenCV v38 CUDA native include sysroot fix if opencv is active"
    ...
    if ! grep -q 'jetsonbuilder38' /tmp/jetson-builder-opencv-env.txt; then
        die "OpenCV is active, but the v38 CUDA sysroot bbappend is not active in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_fix_opencv_cuda_native_include' /tmp/jetson-builder-opencv-env.txt; then
        die "OpenCV v38 CUDA native include hook is not visible in BitBake metadata."
    fi
    bitbake -c clean opencv || true

...
###############################################################################
# build_oe4t_jetson_multi_platform_v16.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v16.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v16.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v16.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v16.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v16.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v16.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v16.sh
#
# Important:
...
ensure_builder_fix_layer() {
    ...
    mkdir -p \
        ...
        "$fix_layer/recipes-bsp/tegra-flashtools"

    cat > "$fix_layer/conf/layer.conf" <<EOF_LAYER
# Local fixes generated by build_oe4t_jetson_multi_platform_v16.sh.
...
# On aarch64 hosts, the NVIDIA BSP payload can install a prebuilt mkbootimg
# helper into the native tegra-flash tools directory that is not executable by
# the build host. That later fails during tegra-minimal-initramfs do_image_cpio.
#
# V15 assumed the helper was always x86_64 and forced qemu-x86_64. The next
# failure showed qemu-x86_64 rejecting the real payload as "Invalid ELF image",
# so this version detects the staged helper architecture and falls back through
# the matching qemu user-mode emulator only when direct execution is not
# possible. The workaround remains scoped to tegra-flashtools-native.
...
do_install:append() {
    ...
    # Idempotent for rebuilds after a partially completed workdir.
    if [ -f "${mkbootimg_path}.real" ]; then
        return
    fi

    mv "$mkbootimg_path" "${mkbootimg_path}.real"

    cat > "$mkbootimg_path" <<'EOF_MKBOOTIMG_WRAPPER'
#!/bin/sh
set -u
...
elif [ -x "$native_bindir/file" ]; then
    magic="$($native_bindir/file -L "$real" 2>/dev/null || true)"
fi

run_direct_if_reasonable() {
    case "$magic" in
        *"shell script"*|*"Python script"*|*"Perl script"*)
            exec "$real" "$@"
            ;;
        *"ELF"*"x86-64"*)
            case "$host_arch" in x86_64|amd64) exec "$real" "$@" ;; esac
            ;;
        *"ELF"*"aarch64"*|*"ELF"*"ARM aarch64"*)
            case "$host_arch" in aarch64|arm64) exec "$real" "$@" ;; esac
            ;;
        *"ELF"*"ARM"*)
            case "$host_arch" in armv7*|armv6*|armhf|arm) exec "$real" "$@" ;; esac
            ;;
        *"ELF"*"80386"*|*"ELF"*"Intel 80386"*)
            case "$host_arch" in i386|i486|i586|i686) exec "$real" "$@" ;; esac
            ;;
        "")
            # Unknown because file(1) is unavailable. Try direct execution and
            # only fall back for shell's standard "cannot execute" statuses.
            "$real" "$@"
            rc=$?
            case "$rc" in 126|127) return 1 ;; *) exit "$rc" ;; esac
            ;;
        *)
            # Non-ELF or unknown non-script. Try direct once.
            "$real" "$@"
            rc=$?
            case "$rc" in 126|127) return 1 ;; *) exit "$rc" ;; esac
            ;;
...
case "$magic" in
    *"ELF"*"x86-64"*)
        find_and_exec_qemu qemu-x86_64 "$@" || true
        ;;
    *"ELF"*"80386"*|*"ELF"*"Intel 80386"*)
        find_and_exec_qemu qemu-i386 "$@" || true
        ;;
    *"ELF"*"aarch64"*|*"ELF"*"ARM aarch64"*)
        find_and_exec_qemu qemu-aarch64 "$@" || true
        ;;
    *"ELF"*"ARM"*)
        find_and_exec_qemu qemu-arm "$@" || true
        ;;
    *)
        # Last-resort fallback for vendor payloads whose architecture string is
        # unusual or when file(1) was unavailable in the task environment.
        for qemu_name in qemu-x86_64 qemu-i386 qemu-aarch64 qemu-arm; do
            find_and_exec_qemu "$qemu_name" "$@" || true
        done
        ;;
esac
...
EOF_MKBOOTIMG_WRAPPER
    ...
    chmod 0755 "$mkbootimg_path"
    chmod 0755 "${mkbootimg_path}.real"
}
EOF_TEGRA_FLASHTOOLS'
# On aarch64 hosts, the NVIDIA BSP payload can install an x86_64 mkbootimg
# helper into the native tegra-flash tools directory. That later fails during
# tegra-minimal-initramfs do_image_cpio with:
#   mkbootimg: Exec format error
#
# Keep this workaround scoped to tegra-flashtools-native. Rename the opaque
# vendor helper and install a wrapper that either executes it directly on x86_64
# hosts or runs it through qemu-x86_64 on aarch64/other hosts.
...
if [ ! -x "$real" ]; then
    echo "mkbootimg wrapper error: missing executable $real" >&2
    exit 126
fi
...
# Prefer qemu from the recipe-native sysroot. mkbootimg lives at:
#   recipe-sysroot-native/usr/bin/tegra-flash/mkbootimg
# so ../qemu-x86_64 resolves to recipe-sysroot-native/usr/bin/qemu-x86_64.
tool_dir="$(CDPATH= cd -- "$(dirname -- "$self")" && pwd)"
native_bindir="$(CDPATH= cd -- "$tool_dir/.." && pwd)"

for qemu in \
    "$native_bindir/qemu-x86_64" \
    "$native_bindir/qemu-x86_64-static" \
    qemu-x86_64 \
    qemu-x86_64-static
 do
    case "$qemu" in
...
echo "mkbootimg wrapper error: $real is not executable on $host_arch and qemu-x86_64 was not found" >&2
exit 126
EOF_MKBOOTIMG_WRAPPER

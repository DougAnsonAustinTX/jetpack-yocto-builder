...
###############################################################################
# build_oe4t_jetson_multi_platform_v28.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v28.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v28.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v28.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v28.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v28.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v28.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v28.sh
#
# Important:
...
#   - Removes obsolete SSTATE_DUPWHITELIST usage entirely because current
#     BitBake aborts during parse when that renamed variable appears.
#   - Keeps only SSTATE_ALLOW_OVERLAP_FILES for Thor CUDA header overlaps,
#     with exact CUDA 13 fatbinary_section.h paths instead of relying only
#     on wildcards.
#   - Bumps the Thor CUDA cleansstate stamp to v28 and includes local.conf in
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v28.sh thor cuda overlap guard
...
# parse error, so v28 uses only the renamed variable.
...
# END generated by build_oe4t_jetson_multi_platform_v28.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v28.sh.
BBPATH .= ":\${LAYERDIR}"
...
# v28 prunes both component files and sstate-control manifest lines. The latter
...
python jetson_builder_prune_cuda_nvcc_native_component () {
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            if changed:
                tmp_manifest = manifest + '.jetsonbuilder28.tmp'
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v28.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v28 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA native state so the v28 nvcc/header collision fix is used"

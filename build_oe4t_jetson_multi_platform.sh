...
###############################################################################
# build_oe4t_jetson_multi_platform_v50.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v50.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v50.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v50.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v50.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v50.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v50.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v50.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v50.sh --bundle
#
# Important:
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v50.sh
  ./build_oe4t_jetson_multi_platform_v50.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v50.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v50.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v50.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v50.sh.
PR:append = ".jetsonbuilder50"
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v50.sh.
BBPATH .= ":\${LAYERDIR}"
...
jetson_builder_libidn2_sanitize_unistring_makefiles() {
    for mk in "${B}/unistring/Makefile"; do
        [ -f "${mk}" ] || continue
        cp -p "${mk}" "${mk}.jetsonbuilder50.bak" 2>/dev/null || true
        sed -i \
...
jetson_builder_libidn2_remove_stale_unistring_outputs() {
...
#
# v50 only serialized perl-native. The uploaded log shows target perl itself
...
clean_perl_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "perl-target-parallel-clean-v50" "perl perl-native" \
...
clean_libidn2_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "libidn2-unistring-install-clean-v50" "libidn2" \
        "$fix_layer/recipes-support/libidn2/libidn2_%.bbappend"
}

validate_libidn2_unistring_fix() {
    log "Validating libidn2 v50 unistring install guard is visible"
    ...
    grep -q 'jetson_builder_libidn2_sanitize_unistring_makefiles' /tmp/jetson-builder-libidn2-env.txt || die "libidn2 is active, but the v50 unistring Makefile sanitizer hook is not visible in BitBake metadata."
}

validate_perl_parallel_fix() {
    log "Validating perl v50 target parallelism guard is visible"
    ...
    die "perl is active, but the v50 PARALLEL_MAKE=-j1 guard is not visible in BitBake metadata."
}

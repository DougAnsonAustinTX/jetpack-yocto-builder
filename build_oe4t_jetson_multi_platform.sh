...
###############################################################################
# build_oe4t_jetson_multi_platform_v35.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v35.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v35.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v35.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v35.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v35.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v35.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v35.sh
#
# Important:
...
#   - Bumps the Thor CUDA clean/repair stamp to v35 so this runs even if v28
#     already created stamps.
#   - Adds the FreeImage 3.18.0 -fPIC bbappend only after BitBake confirms
#     that a freeimage recipe is active. v35 fixes v33 by preserving FreeImage's
#     original include/macro flags and appending PIC through Makefile.gnu instead
#     of replacing CFLAGS/CXXFLAGS on the make command line.
###############################################################################
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v35.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v35.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v35.sh.
BBPATH .= ":\${LAYERDIR}"
...
# v35 is intentionally surgical: remove only the known duplicate header from
...
PR:append = ".jetsonbuilder35"

jetson_builder_prune_cuda_nvcc_duplicate_header_from_dir() {
    root_base="$1"
    for duplicate in         "${root_base}/usr/local/cuda-13.0/targets/sbsa-linux/include/fatbinary_section.h"         "${root_base}${prefix}/local/cuda-13.0/targets/sbsa-linux/include/fatbinary_section.h"; do
...
# v35 removes only the exact duplicate fatbinary_section.h ownership entry from
# cuda-nvcc-native. Broader include-directory pruning broke OpenCV because
...
python jetson_builder_prune_cuda_nvcc_native_component () {
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            if changed:
                tmp_manifest = manifest + '.jetsonbuilder35.tmp'
...
# Generated by build_oe4t_jetson_multi_platform_v35.sh.
# FreeImage 3.18.0 uses command-line/exported CFLAGS from Yocto. On this Thor
# branch those flags already contain the required FreeImage macro/include flags
# from Makefile.srcs, but they do not contain -fPIC. Do NOT override do_compile
# with hand-built CFLAGS, because that drops INCLUDE and breaks FreeImage.h.
# Instead patch Makefile.gnu so GNU make appends PIC to the effective flags.
PR:append = ".jetsonbuilder35"

jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        # Remove older generated blocks from previous local attempts if the
        # workdir was reused for any reason.
        sed -i \
...
# jetson-builder v35 force PIC without replacing FreeImage include/macro flags
...
ensure_active_freeimage_pic_fix() {
    ...
    local env_file="/tmp/jetson-builder-freeimage-v35-env.txt"
    if ! bitbake -e freeimage > "$env_file" 2>/tmp/jetson-builder-freeimage-v35-env.err; then
    ...
    if [ -n "$recipe_file" ] && [ -f "$recipe_file" ]; then
        ...
        log "Installing FreeImage v35 PIC bbappend next to active recipe: $recipe_file"
...
for manifest in manifest_candidates:
    ...
    for line in lines:
        ...
        kept.append(line)
    if changed:
        tmp_manifest = manifest.with_suffix(manifest.suffix + '.jetsonbuilder35.tmp')
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v35.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v35 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA native state so the v35 exact-header collision fix is used"
    bitbake -c cleansstate cuda-nvcc-native cuda-nvcc-headers-native cuda-compiler-native
    # OpenCV may have a recipe-sysroot-native populated by the older broad-prune
    # CUDA fix. Clean its workdir so CMake sees the restored CUDA include path.
...
clean_freeimage_state_if_needed() {
    ...
    if ! bitbake -e freeimage >/tmp/jetson-builder-freeimage-v35-clean-env.txt 2>/dev/null; then
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v35.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v35 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v35 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    ...
    log "Validating FreeImage v35 PIC fix only if freeimage is active"
    ...
    if ! grep -q 'jetsonbuilder35' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage is active, but the v35 PIC bbappend is not active in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_force_freeimage_pic' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v35 PIC patch hook is not visible in BitBake metadata."
    fi

    log "Forcing FreeImage compile validation with v35 PIC fix"
    bitbake -c cleansstate freeimage
...
validate_opencv_cuda_sysroot_fix() {
    ...
    log "Validating OpenCV configure sees restored native CUDA include path if opencv is active"

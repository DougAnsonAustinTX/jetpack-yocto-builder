...
###############################################################################
# build_oe4t_jetson_multi_platform_v29.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v29.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v29.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v29.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v29.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v29.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v29.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v29.sh
#
# Important:
...
#   - Bumps the Thor CUDA clean/repair stamp to v29 so this runs even if v28
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v29.sh thor cuda overlap guard
...
# parse error, so v29 uses only the renamed variable.
...
# END generated by build_oe4t_jetson_multi_platform_v29.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v29.sh.
BBPATH .= ":\${LAYERDIR}"
...
# v29 prunes both component files and sstate-control manifest lines. The latter
...
python jetson_builder_prune_cuda_nvcc_native_component () {
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            if changed:
                tmp_manifest = manifest + '.jetsonbuilder29.tmp'
...
for manifest in manifest_candidates:
    ...
    for line in lines:
        ...
        kept.append(line)
    if changed:
        tmp_manifest = manifest.with_suffix(manifest.suffix + '.jetsonbuilder29.tmp')
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v29.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v29 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA native state so the v29 nvcc/header collision fix is used"

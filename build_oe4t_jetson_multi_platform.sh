...
###############################################################################
# build_oe4t_jetson_multi_platform_v68.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v68.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v68.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v68.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v68.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v68.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v68.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v68.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v68.sh --bundle
#
# Important:
...
#
# v68 notes:
#   - Full standalone script release. This file is not a launcher and does not
#     depend on any previous v65/v66/v67 script being present beside it.
...
#   - Uses a new v68 clean stamp and validates the populated native sysroot so
#     stale unwrapped tegra-flashtools-native content is discarded.
#
# v65 notes:
#   - Fixes Thor tegra-minimal-initramfs do_image_cpio failure:
#       mkbootimg: Exec format error
#     The v64 wrapper only targeted a standalone tegra-flashtools-native recipe,
#     but this OE4T Thor stack uses tegra-flashtools_38.2.1.bb with
#     BBCLASSEXTEND/native, so the native sysroot still received NVIDIA's
#     foreign-architecture mkbootimg directly.
#   - Adds the mkbootimg wrapper to the tegra-flashtools base recipe's
#     class-native install path, keeps support for standalone
#     tegra-flashtools-native where present, and cleans stale
#     tegra-flashtools-native state when the wrapper changes.
#
# v64 notes:
#   - Fixes the v63 libcxx-native do_configure regression where the generated
#     libcxx bbappend used shell-default expressions such as
#     ${OECMAKE_SOURCEPATH:-}. BitBake did not expand those expressions into
#     recipe values, so the guard saw OECMAKE_SOURCEPATH/S/TMPDIR as empty and
#     searched /work-shared before fataling.
#   - Uses direct BitBake variable expansion for libcxx, matching the working
#     compiler-rt guard style, and adds WORKDIR-derived TMPDIR fallback logic.
#   - Bumps the LLVM missing-runtimes cleansstate stamp so stale v63 task state
#     is discarded automatically.
#
# v63 notes:
#   - Fixes v62's dangling bbappend regression:
#       ERROR: No recipes in default available for .../libcxxabi_%.bbappend
#     v62 generated speculative libcxxabi/libunwind bbappends even though this
#     Thor layer stack does not expose matching standalone recipes. v63 removes
#     those bbappends and deletes stale copies left by v62 before BitBake parses.
#   - Keeps the actual verified missing-runtimes repairs on libcxx and
#     compiler-rt only. These are the recipes observed to fail and the recipes
#     known to exist in this build.
#   - Keeps the broader PR lesson from v62: generated compatibility bbappends
#     must not append PR for shared-source LLVM runtime consumers.
#
# v62 notes:
#   - Removed local PR:append mutations from generated compatibility bbappends.
...
#   - Attempted to generalize LLVM missing-runtimes coverage to libcxxabi and
#     libunwind, but that created dangling bbappends on layer stacks where those
#     are not standalone recipes. v63 removes that over-broadening.
#
# v61 notes:
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v68.sh
  ./build_oe4t_jetson_multi_platform_v68.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v68.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v68.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v68.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v68.sh.
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v68.sh.
BBPATH .= ":\${LAYERDIR}"
...
jetson_builder_libcxx_repair_missing_runtimes_source() {
    ...
    # Use direct BitBake expansion here. Do not use shell-default forms such as
    # shell colon-default expansion; BitBake leaves that for the shell, where these
    # variables are not guaranteed to exist, and the guard then sees empty paths.
    ...
    if [ -z "$bitbake_tmpdir" ] && [ -n "$workdir" ]; then
        case "$workdir" in
            */work/*) bitbake_tmpdir="${workdir%/work/*}" ;;
        esac
    fi
...
"
            ...
            bbnote "jetson-builder v64: candidate LLVM source root from BitBake variable does not exist for ${PN}: $maybe_root"
        fi
    done
...
"
        ...
        bbnote "jetson-builder v64: TMPDIR/work-shared is not visible for ${PN}: $bitbake_tmpdir/work-shared"
    fi
    ...
    if [ -z "$found_root" ] || [ ! -d "$found_root" ]; then
        bbfatal "jetson-builder v64: no usable shared llvm-project runtimes source root found for ${PN}; OECMAKE_SOURCEPATH='$oecmake_sourcepath' S='$recipe_s' TMPDIR='$bitbake_tmpdir'"
    fi

    real_runtimes_dir="$found_root/runtimes"
    bbnote "jetson-builder v64: selected LLVM source root for ${PN}: $found_root"

    if [ -f "$real_runtimes_dir/CMakeLists.txt" ]; then
        bbnote "jetson-builder v64: LLVM runtimes source already exists for ${PN}: $real_runtimes_dir"
        ...
        bbnote "jetson-builder v64: linked missing LLVM runtimes source for ${PN}: $real_runtimes_dir -> $found_root/llvm/runtimes"
    else
        bbfatal "jetson-builder v64: selected LLVM source root has no runtimes CMake entry point for ${PN}: $found_root"
    fi
    ...
    if [ -n "$cmake_runtimes_dir" ] && [ "$cmake_runtimes_dir" != "$real_runtimes_dir" ]; then
        if [ -e "$cmake_runtimes_dir" ] || [ -L "$cmake_runtimes_dir" ]; then
            if [ -f "$cmake_runtimes_dir/CMakeLists.txt" ]; then
                bbnote "jetson-builder v64: exact CMake runtimes path already exists for ${PN}: $cmake_runtimes_dir"
                return 0
            fi
        ...
        bbnote "jetson-builder v64: linked exact CMake runtimes source path for ${PN}: $cmake_runtimes_dir -> $real_runtimes_dir"
    fi
}
...
# llvm-project .../runtimes CMake entry point. If local metadata mutates recipe
# path variables, CMake can be invoked on a path that is different from the real
# unpacked shared source tree. Repair the real tree first, then make the exact
# CMake source path exist as well.
jetson_builder_compiler_rt_make_empty_runtimes_tree() {
    target_runtimes_dir="$1"
    source_root="$2"
    recipe_s="$3"

    rm -rf "$target_runtimes_dir"
    mkdir -p "$target_runtimes_dir"

    compiler_rt_src=""
    for candidate in "$source_root/compiler-rt" "$recipe_s/compiler-rt" "$recipe_s"; do
    ...
    if [ -n "$compiler_rt_src" ]; then
        ln -snf "$compiler_rt_src" "$target_runtimes_dir/compiler-rt"
        for runtime in libunwind libcxx libcxxabi; do
            if [ -d "$source_root/$runtime" ]; then
                ln -snf "$source_root/$runtime" "$target_runtimes_dir/$runtime"
            fi
        done
        bbnote "jetson-builder v64: created fallback LLVM runtimes entry point for ${PN} at $target_runtimes_dir; individual runtime sources were linked for diagnostics"
    else
        bbwarn "jetson-builder v64: compiler-rt source was not found under $source_root; creating empty compiler-rt fallback for ${PN} at $target_runtimes_dir"
    fi
...
add_custom_target(compiler_rt_empty ALL
    COMMAND ${CMAKE_COMMAND} -E echo "jetson-builder v64: empty compiler-rt fallback")
install(CODE "message(STATUS \"jetson-builder v64: installing empty compiler-rt fallback\")")
...
"
    ...
    if [ -z "$found_root" ] || [ ! -d "$found_root" ]; then
        bbwarn "jetson-builder v64: no shared llvm-project source root found for ${PN}; OECMAKE_SOURCEPATH='$oecmake_sourcepath' S='$recipe_s' TMPDIR='$bitbake_tmpdir'"
        return 0
    fi
...
EOF_COMPILER_RT_RPROVIDES
    ...
    # v63: do not generate speculative libcxxabi/libunwind bbappends.
    # This Thor layer stack does not expose matching standalone recipes, and
    # BitBake treats unmatched bbappends as fatal. Remove stale v62 copies before
    # any parse/validation step can see them.
    rm -f \
...
clean_recipe_state_if_fix_changed() {
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "$stamp_name cleansstate guard already applied for the current v68 fix."
        return 0
    fi
...
clean_compiler_rt_runtimes_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "llvm-missing-runtimes-clean-v64" "libcxx libcxx-native compiler-rt compiler-rt-native" \
        "$fix_layer/recipes-devtools/clang/libcxx_%.bbappend" \
        "$fix_layer/recipes-devtools/clang/compiler-rt_%.bbappend"
}
...
validate_compiler_rt_runtimes_fix() {
    log "Validating LLVM v64 missing-runtimes configure guards are visible"

    local recipe env_file err_file guard
    for recipe in libcxx-native compiler-rt-native; do
        ...
        grep -q "$guard" "$env_file" || die "$recipe is active, but the v64 missing-runtimes configure guard is not visible in BitBake metadata."
        if [ "$recipe" = "compiler-rt-native" ]; then
            grep -q 'JetsonBuilderCompilerRTFallback' "$env_file" || die "compiler-rt-native is active, but the v64 empty fallback CMake guard is not visible in BitBake metadata."
        fi
    done
}
...
clean_tegra_flashtools_state_if_needed() {
    local platform="$1" distro_dir="$2" build_abs_dir="$3"
    [ "$platform" = "thor" ] || return 0
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "tegra-flashtools-flash-helper-wrapper-clean-v68" "tegra-flashtools tegra-flashtools-native" \
...
validate_tegra_flashtools_wrappers() {
    local platform="$1" build_abs_dir="${2:-}"
    [ "$platform" = "thor" ] || return 0
    log "Validating Thor v68 tegra-flashtools native wrappers for direct and tegra-flash mkbootimg/mksparse helpers"
    if ! bitbake -e tegra-flashtools-native >/tmp/jetson-builder-tegra-flashtools-native-v68-env.txt 2>/tmp/jetson-builder-tegra-flashtools-native-v68-env.err; then
        ...
        grep -q 'jetson_builder_wrap_tegra_flash_helper_path' /tmp/jetson-builder-tegra-flashtools-native-v68-env.txt || die "tegra-flashtools-native is active, but the v67 helper wrapper install hook is not visible in BitBake metadata."
    fi
    ...
    if [ -n "$build_abs_dir" ] && [ -d "$build_abs_dir/tmp/sysroots-components" ]; then
        ...
        while IFS= read -r helper; do
            ...
            warn "Thor v68 validation: unwrapped tegra-flashtools helper remains in native sysroot: $helper"
            bad=1
        ...
        [ "$found" = "1" ] || warn "Thor v68 validation: no mkbootimg/mksparse helpers found under tegra-flashtools-native sysroots-components yet."
        [ "$bad" = "0" ] || die "Thor v68 tegra-flashtools validation failed; refusing to continue with unwrapped foreign-architecture helper payloads."
    fi
}
...
clean_freeimage_state_if_needed() {
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v68 fix."
        return 0
    fi
...
EOF_BUILD_CONFIG
    ...
    clean_tegra_flashtools_state_if_needed "$platform" "$workspace/tegra-demo-distro" "$build_abs_dir"

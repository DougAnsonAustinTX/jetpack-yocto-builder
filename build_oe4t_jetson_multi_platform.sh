...
###############################################################################
# build_oe4t_jetson_multi_platform_v36.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v36.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v36.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v36.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v36.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v36.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v36.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v36.sh
#
# Important:
...
#     headers exist. v36 creates a recipe-local native CUDA include symlink back
#     to the target recipe sysroot before OpenCV CMake generation validates the
#     imported targets.
#   - Bumps all Thor CUDA/OpenCV/FreeImage stamps and PR suffixes to v36 so the
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v36.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v36.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v36.sh.
...
jetson_builder_fix_opencv_cuda_native_include() {
    ...
    echo "jetson-builder v36 OpenCV CUDA native root: ${native_root}"
    echo "jetson-builder v36 OpenCV CUDA target root: ${target_root}"
    echo "jetson-builder v36 OpenCV CUDA selected target include: ${target_include:-none}"
    ls -ld "${native_root}" || true
    ls -ld "${native_include}" || true

    if [ ! -d "${native_include}" ]; then
        bbfatal "jetson-builder v36: ${native_include} still does not exist; cannot satisfy CMake CUDA imported target validation"
    fi
}
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v36.sh.
BBPATH .= ":\${LAYERDIR}"
...
# v36 is intentionally surgical: remove only the known duplicate header from
...
# v36 removes only the exact duplicate fatbinary_section.h ownership entry from
...
python jetson_builder_prune_cuda_nvcc_native_component () {
    ...
    for pattern in manifest_patterns:
        for manifest in glob.glob(pattern):
            ...
            if changed:
                tmp_manifest = manifest + '.jetsonbuilder36.tmp'
...
EOF_CUDA_COMPILER
    ...
    local opencv_recipe_pattern="$distro_dir/layers/meta-oe/recipes-support/opencv/opencv_"*.bb
    if [ "$platform" = "thor" ] && compgen -G "$opencv_recipe_pattern" >/dev/null; then
        ...
        if [ "$platform" = "thor" ]; then
            warn "Skipping Thor OpenCV CUDA sysroot fix; no matching OpenCV recipe was found in meta-oe."
        fi
    fi
...
jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        sed -i \
            ...
            -e '/jetson-builder v36 force PIC/,+4d' \
            "${S}/Makefile.gnu"

        cat >> "${S}/Makefile.gnu" <<'EOF_MAKEFILE_PIC'

# jetson-builder v36 force PIC without replacing FreeImage include/macro flags
...
ensure_active_freeimage_pic_fix() {
    ...
    local env_file="/tmp/jetson-builder-freeimage-v36-env.txt"
    if ! bitbake -e freeimage > "$env_file" 2>/tmp/jetson-builder-freeimage-v36-env.err; then
    ...
    if [ -n "$recipe_file" ] && [ -f "$recipe_file" ]; then
        ...
        log "Installing FreeImage v36 PIC bbappend next to active recipe: $recipe_file"
...
for manifest in manifest_candidates:
    ...
    for line in lines:
        ...
        kept.append(line)
    if changed:
        tmp_manifest = manifest.with_suffix(manifest.suffix + '.jetsonbuilder36.tmp')
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v36.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v36 fix."
        return 0
    fi

    log "Cleaning stale Thor CUDA/OpenCV state so the v36 CUDA include repair is used"
...
clean_freeimage_state_if_needed() {
    ...
    if ! bitbake -e freeimage >/tmp/jetson-builder-freeimage-v36-clean-env.txt 2>/dev/null; then
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v36.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v36 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v36 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    ...
    log "Validating FreeImage v36 PIC fix only if freeimage is active"
    ...
    if ! grep -q 'jetsonbuilder36' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage is active, but the v36 PIC bbappend is not active in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_force_freeimage_pic' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v36 PIC patch hook is not visible in BitBake metadata."
    fi

    log "Forcing FreeImage compile validation with v36 PIC fix"
    bitbake -c cleansstate freeimage
...
validate_opencv_cuda_sysroot_fix() {
    ...
    log "Validating OpenCV v36 CUDA native include sysroot fix if opencv is active"
    ...
    if ! grep -q 'jetsonbuilder36' /tmp/jetson-builder-opencv-env.txt; then
        die "OpenCV is active, but the v36 CUDA sysroot bbappend is not active in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_fix_opencv_cuda_native_include' /tmp/jetson-builder-opencv-env.txt; then
        die "OpenCV v36 CUDA native include hook is not visible in BitBake metadata."
    fi
    bitbake -c clean opencv || true

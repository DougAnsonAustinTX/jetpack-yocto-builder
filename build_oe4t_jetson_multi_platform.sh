...
###############################################################################
# build_oe4t_jetson_multi_platform_v63.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v63.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v63.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v63.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v63.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v63.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v63.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v63.sh
#
# Bundle primary tegraflash artifact after a successful build:
#   ./build_oe4t_jetson_multi_platform_v63.sh --bundle
#
# Important:
...
Examples:
  ...
  BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v63.sh
  ./build_oe4t_jetson_multi_platform_v63.sh --bundle
  TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v63.sh --bundle
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v63.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v63.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Generated by build_oe4t_jetson_multi_platform_v63.sh.
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v63.sh.
BBPATH .= ":\${LAYERDIR}"
...
"
            ...
            bbnote "jetson-builder v63: candidate LLVM source root from BitBake variable does not exist for ${PN}: $maybe_root"
        fi
    done
...
"
        ...
        bbnote "jetson-builder v63: TMPDIR/work-shared is not visible for ${PN}: $bitbake_tmpdir/work-shared"
    fi
    ...
    if [ -z "$found_root" ] || [ ! -d "$found_root" ]; then
        bbfatal "jetson-builder v63: no usable shared llvm-project runtimes source root found for ${PN}; OECMAKE_SOURCEPATH='$oecmake_sourcepath' S='$recipe_s' TMPDIR='$bitbake_tmpdir'"
    fi

    real_runtimes_dir="$found_root/runtimes"
    bbnote "jetson-builder v63: selected LLVM source root for ${PN}: $found_root"

    if [ -f "$real_runtimes_dir/CMakeLists.txt" ]; then
        bbnote "jetson-builder v63: LLVM runtimes source already exists for ${PN}: $real_runtimes_dir"
        ...
        bbnote "jetson-builder v63: linked missing LLVM runtimes source for ${PN}: $real_runtimes_dir -> $found_root/llvm/runtimes"
    else
        bbfatal "jetson-builder v63: selected LLVM source root has no runtimes CMake entry point for ${PN}: $found_root"
    fi
    ...
    if [ -n "$cmake_runtimes_dir" ] && [ "$cmake_runtimes_dir" != "$real_runtimes_dir" ]; then
        if [ -e "$cmake_runtimes_dir" ] || [ -L "$cmake_runtimes_dir" ]; then
            if [ -f "$cmake_runtimes_dir/CMakeLists.txt" ]; then
                bbnote "jetson-builder v63: exact CMake runtimes path already exists for ${PN}: $cmake_runtimes_dir"
                return 0
            fi
        ...
        bbnote "jetson-builder v63: linked exact CMake runtimes source path for ${PN}: $cmake_runtimes_dir -> $real_runtimes_dir"
    fi
}
...
jetson_builder_compiler_rt_make_empty_runtimes_tree() {
    ...
    if [ -n "$compiler_rt_src" ]; then
        ...
        bbnote "jetson-builder v63: created fallback LLVM runtimes entry point for ${PN} at $target_runtimes_dir; individual runtime sources were linked for diagnostics"
    else
        bbwarn "jetson-builder v63: compiler-rt source was not found under $source_root; creating empty compiler-rt fallback for ${PN} at $target_runtimes_dir"
    fi
...
add_custom_target(compiler_rt_empty ALL
    COMMAND ${CMAKE_COMMAND} -E echo "jetson-builder v63: empty compiler-rt fallback")
install(CODE "message(STATUS \"jetson-builder v63: installing empty compiler-rt fallback\")")
...
"
    ...
    if [ -z "$found_root" ] || [ ! -d "$found_root" ]; then
        bbwarn "jetson-builder v63: no shared llvm-project source root found for ${PN}; OECMAKE_SOURCEPATH='$oecmake_sourcepath' S='$recipe_s' TMPDIR='$bitbake_tmpdir'"
        return 0
    fi
...
clean_compiler_rt_runtimes_state_if_needed() {
    local distro_dir="$1" build_abs_dir="$2"
    local fix_layer="$distro_dir/layers/meta-jetson-builder-fixes"
    clean_recipe_state_if_fix_changed "$build_abs_dir" "llvm-missing-runtimes-clean-v63" "libcxx libcxx-native compiler-rt compiler-rt-native" \
        "$fix_layer/recipes-devtools/clang/libcxx_%.bbappend" \
        "$fix_layer/recipes-devtools/clang/compiler-rt_%.bbappend"
}
...
validate_compiler_rt_runtimes_fix() {
    log "Validating LLVM v63 missing-runtimes configure guards are visible"
    ...
    for recipe in libcxx-native compiler-rt-native; do
        ...
        grep -q "$guard" "$env_file" || die "$recipe is active, but the v63 missing-runtimes configure guard is not visible in BitBake metadata."
        if [ "$recipe" = "compiler-rt-native" ]; then
            grep -q 'JetsonBuilderCompilerRTFallback' "$env_file" || die "compiler-rt-native is active, but the v63 empty fallback CMake guard is not visible in BitBake metadata."
        fi
    done
}

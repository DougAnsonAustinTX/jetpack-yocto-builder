...
###############################################################################
# build_oe4t_jetson_multi_platform_v34.sh
#
...
# Build one platform:
#   ./build_oe4t_jetson_multi_platform_v34.sh
#   TARGET_PLATFORM=orin-nx ./build_oe4t_jetson_multi_platform_v34.sh
#   TARGET_PLATFORM=thor ./build_oe4t_jetson_multi_platform_v34.sh
#
# Build all three platforms:
#   BUILD_ALL_PLATFORMS=1 ./build_oe4t_jetson_multi_platform_v34.sh
#
# Smaller image:
#   TARGET_IMAGE=demo-image-base ./build_oe4t_jetson_multi_platform_v34.sh
#
# Clean one platform build directory:
#   CLEAN_BUILD=1 ./build_oe4t_jetson_multi_platform_v34.sh
#
# Production-ish build without permissive dev login:
#   DEV_LOGIN_FEATURES=0 ./build_oe4t_jetson_multi_platform_v34.sh
#
# Important:
...
#   - Bumps the Thor CUDA clean/repair stamp to v34 so this runs even if v28
#     already created stamps.
#   - Adds the FreeImage 3.18.0 -fPIC bbappend only after BitBake confirms
#     that a freeimage recipe is active. v34 fixes v33 by preserving FreeImage's
...
# BEGIN generated by build_oe4t_jetson_multi_platform_v34.sh thor cuda overlap guard
...
# END generated by build_oe4t_jetson_multi_platform_v34.sh thor cuda overlap guard
EOF_THOR_CUDA_CONF
    fi
...
# Local fixes generated by build_oe4t_jetson_multi_platform_v34.sh.
BBPATH .= ":\${LAYERDIR}"
...
# Generated by build_oe4t_jetson_multi_platform_v34.sh.
...
jetson_builder_force_freeimage_pic() {
    if [ -f "${S}/Makefile.gnu" ]; then
        ...
        sed -i \
            ...
            -e '/jetson-builder v34 force PIC/,+4d' \
            "${S}/Makefile.gnu"

        cat >> "${S}/Makefile.gnu" <<'EOF_MAKEFILE_PIC'

# jetson-builder v34 force PIC without replacing FreeImage include/macro flags
...
ensure_active_freeimage_pic_fix() {
    ...
    local env_file="/tmp/jetson-builder-freeimage-v34-env.txt"
    if ! bitbake -e freeimage > "$env_file" 2>/tmp/jetson-builder-freeimage-v34-env.err; then
    ...
    if [ -n "$recipe_file" ] && [ -f "$recipe_file" ]; then
        ...
        log "Installing FreeImage v34 PIC bbappend next to active recipe: $recipe_file"
...
clean_thor_cuda_native_state_if_needed() {
    ...
    local stamp_file="$stamp_dir/thor-cuda-native-clean-v34.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "Thor CUDA native cleansstate guard already applied for the current v34 fix."
        return 0
    fi
...
clean_freeimage_state_if_needed() {
    ...
    if ! bitbake -e freeimage >/tmp/jetson-builder-freeimage-v34-clean-env.txt 2>/dev/null; then
    ...
    local stamp_file="$stamp_dir/freeimage-pic-clean-v34.sha256"
    mkdir -p "$stamp_dir"
    ...
    if [ -f "$stamp_file" ] && grep -qxF "$current_hash" "$stamp_file"; then
        log "FreeImage PIC cleansstate guard already applied for the current v34 fix."
        return 0
    fi

    log "Cleaning stale FreeImage state so the v34 -fPIC fix is used"
...
validate_freeimage_pic_fix() {
    ...
    log "Validating FreeImage v34 PIC fix only if freeimage is active"
    ...
    if ! grep -q 'jetsonbuilder34' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage is active, but the v34 PIC bbappend is not active in BitBake metadata."
    fi
    if ! grep -q 'jetson_builder_force_freeimage_pic' /tmp/jetson-builder-freeimage-env.txt; then
        die "FreeImage v34 PIC patch hook is not visible in BitBake metadata."
    fi

    log "Forcing FreeImage compile validation with v34 PIC fix"
    bitbake -c cleansstate freeimage
